...
#              import pdb;pdb.set_trace(); #for debug
              fnsm='{}/{}.dat'.format(dr,sm)
#              if os.path.exists(fnout) and args.ow == 0:
              if not os.path.exists(fnsm) or (os.path.exists(fnsm) and args.ow >=2):
                print('#speechmix:{}'.format(cmd))
                mylib.myshell(cmd) #speechmix.py
              else:
                print('#{} exists: Omit calc because -ow {}(<2)'.format(fnsm,args.ow))
              fnout='{}/{}-M.dat'.format(dr,sm,args.k,args.l)
              if os.path.exists(fnout) and args.ow == 0:
                print('#{} exists: Omit calc because -ow 0'.format(fnout))
                continue
#              k=args.k;N=args.N;t0=0;t1=400
              cmd='python lpcspectrumenvelope.py -yin {} -k {} -l {} -mp_th {} -fout {} -nDisp {}'.format(fnsm,args.k,args.l,args.mp_th,fnout,nDisp)
