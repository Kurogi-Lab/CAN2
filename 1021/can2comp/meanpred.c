...
//  }
  ...
  for(j=0;j<nfiles;j++){
    ...
    if((fp=fopen(fn_is,"r"))==NULL){
      fprintf(stderr,"#Execute without '%s'\n",fn_is);
      //      fprintf(stderr,"file(%s) could not be opened!\n",fn_is);
      num=0; sigma2[j]=0; meansigma2+=0;//050914
      num0=0;
    }
    else{
...
#define NEWp0
	...
	Ltrain+= square(yt[i]-yp[i]);
      ...
      if(nfiles2>=2) Lvar0 /=((num0+ZERO)*(nfiles2)*(nfiles2-1));//estimated variance of ensemble prediction
...
//      {//error check
//	char buff[256];
//	sprintf(buff,"%.7e %d %.7e %.7e %.7e %d %.7e %.7e %.7e %d #Y^,t,Y,yr,var,c,v,k,b,nvar\n",yp[i],n[i],y[i],yr[i],var[i],cc[i],vv[i],kyorix2[i],bias[i],nVi[i]);
