...
int main(int argc,char *argv[]) {
  ...
  net->msg1="";
  net->fnn="net.dat";
  char fnp[256]={"predict.dat"};
  char fnp0[256]={"predict0.dat"};
  char fnis[256]={"is.dat"};
  char fnm[256]={"mse.dat"};
  char fntt[256]={"train+test.dat"};
  net->fnp=&fnp[0];
  net->fnis=&fnis[0];
  net->fnp0=&fnp0[0];
  net->fnm=&fnm[0];
  ...
  for(i=1;i<argc;i++){
    ...
    else if(strncmp(argv[i],"msg:",4)==0) net->msg1=&argv[i][4];
    else if(strncmp(argv[i],"fnp:",4)==0) {
      char av[256];
      sprintf(av,"%s",&argv[i][4]);
      sprintf(net->fnp,"%spred.dat",av);
      sprintf(net->fnis,"%sis.dat",av);
      sprintf(net->fnm,"%smse.dat",av);
      sprintf(net->fntt,"%strain+test.dat",av);
      char *p=av;
      char *p1=p,*p2;
      for(;*p!=0;p++){
	if(*p=='+'){
	  *p=0;
      ...
      sprintf(net->fnp0,"%s+%spred0.dat",p1,p2);
      //      printf("fnp0=%s\n",net->fnp0);
    }
    else if(strncmp(argv[i],"fnn:",4)==0) net->fnn=&argv[i][4];
    //    else if(strncmp(argv[i],"fnis:",5)==0) net->fnis=&argv[i][5];
  }
...
qu 
  ...
  while (1) {
    ...
    //訓練データの一段予測(single-step prediction)＝学習能力
      ...
      PREDTRAIN=0;
      //      if(net->msg[0]!=0) printf("%s thread%d\n",net->msg,omp_get_thread_num());
      if(net->msg[0]!=0){
	printf("%s %s\n",net->msg,net->msg1);
      }
      else{
