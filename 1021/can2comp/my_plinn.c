 ...
 *====================================================================*/
void init_sort_weights(NET *net, int *s, FLOAT *v, FLOAT *x) {
  FLOAT v_min = 1e+12;
  int i_v_min = 0;
 ...
 * v[i] ：荷重ベクトルcell[i].wと入力ベクトルxとの距離を持つ配列
 *====================================================================*/
void sort_weights(NET *net, int *s, FLOAT *v, int start, int end) {
...
FLOAT cosw0xw1x(FLOAT *w1, FLOAT *w0, FLOAT *x,int k) {
 ...
 *
 *====================================================================*/
void calc_value(NET *net) {//online
...
NET *net_loads(NET *net){
  ...
  for(j=0;j<nEns;j++){
    ...
    net[j].nEns=nEns;
    //    fprintf(stdout,"nEns (Number net files) = %d j=%d\n",nEns,j);
  }
  //  fprintf(stdout,"nEns (Number net files) = %d.\n",nEns);
  return(net);
}
//#endif //
