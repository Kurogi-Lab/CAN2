...
int Task1=0;//Task1=0 for BAGGINGTIMESERIES:task=REGRESSION, task1=TIMESERIES
double *Y0;
///////////////////////////
...
//int setfntraintest(char **fntrain,char **fntest,unsigned int seed,char *fn_givendata,char **argv)
{
  ...
  if(method==-1){//direct validation no-bagging
    ...
    int j;
    for(j=0;j<nFolds;j++){
      //      sprintf(fntrain[j],"./tmp/train%04d.dat",j);
      //      sprintf(fntest[j],"./tmp/test%04d.dat",j);
      sprintf(fntrain[j],"./tmp/train%03d.dat",j);
      sprintf(fntest[j],"./tmp/test%03d.dat",j);
    }
    if(*fupdate=='0') return(0);
    {
      int r=random_seed(seed);
      //#if RAND == ZMTRAND
	...
	init_walkeralias(nGivenData,pbst);
	  ...
	  if(nFolds==1 && fabs(alpha-1)<1e-20){//a=1,b=1 single CAN2
	    printf("#1 Exec Single-CAN2 when nFolds=%d=1 and alpha=%g=1\n",nFolds,alpha);
	    for(n=0;n<nTrainData;n++){
	      for(i=0;i<=K;i++) fprintf(fptrain_xy,"%.7e ",train0[n][i]);
	      fprintf(fptrain_xy,"\n");
	    }
	  }
	  else{
	  ...
	  fclose(fptrain_xy);
      ...
      else {//if(Task==TIMESERIES){ #else if(Task1==1)
        int nn,n;//resample
        ...
        for(j=0;j<nFolds;j++){
          ...
          if(method==BOOTSTRAP){
	    if(nFolds==1 && fabs(alpha-1)<1e-20){//a=1,b=1 single CAN2
	      printf("#2 Exec Single-CAN2 when nFolds=%d and alpha=%g (Task1=%d nTrainData=%d)\n",nFolds,alpha,Task1,nTrainData);
	      for(n=0;n<nTrainData;n++){
		nr[j*nTrainData+n]=n;//resampled n-th data is nn-th data
		nc[j*nGivenData+n]=1;//nn-th data is in training set
	      }
	    }
	    else{
	      for(n=0;n<nTrainData;n++){
	    ...
	    //            printf("#3 Single CAN2? nFolds=%d alpha=%g Task1=%d nFolds1=%d\n",nFolds,alpha,Task1,nFolds1);
        ...
        }
        {
          for(n=0;n<nGivenData;n++){
            ...
            if(feof(fp1)) break;
            if(line1[0]==0) break;
	    //printf("#2 Single CAN2? nFolds=%d alpha=%g Task1=%d\n",nFolds,alpha,Task1);
            for(j=0;j<nFolds;j++){
              if(nc[j*nGivenData+n]!=1){//n-th data is not in training set but in the test
                nTestData[j]++;
                fptest_xy=fopen(fntest[j],"a+");
...
int main(int argc,char **argv)
{
  ...
  char fn_pred[fnsize+17];//char fn_pred[fnsize];
  char fnp[fnsize+9];
  char fn_pred0[fnsize+18];
  char fn_is[fnsize+15];
...
#define ERR2BOOST
  ...
  {//
    ...
    char fnparamj[nFolds][nens][128]; //for parallel processing
    char fnpredj[nFolds][nens][512];
    char fnnetj[nFolds][nens][512];
    //    char cmdj[nFolds][nens][1024];
    char ***cmdj=(char***)malloc(sizeof(char**)*nFolds);
    for(j=0;j<nFolds;j++){
      cmdj[j]=(char**)malloc(sizeof(char*)*nens);
      for(i=0;i<nens;i++) cmdj[j][i]=(char*)malloc(sizeof(char)*1024);
    }
	...
	for(ii=0;ii<nens;ii++){//ensemble with different number of units
	  ...
	  if(((fp=fopen(fn_pred,"r"))!=NULL)&&((fp1=fopen(fn_is,"r"))!=NULL)&&((fp0=fopen(fn_pred0,"r"))!=NULL)){
	    ...
	    fprintf(stderr,"'%s' exists. Omit recalc.\n",fn_pred);
          ...
          else{//if no-prediction-files
	    net_printf(strx1("'%s' getting created!!.\n",fn_pred));
	    char fn_net[fnsize+37];//char fn_net[fnsize];
	    int fn_net_exist=0; 
...
//	    }
	    ...
	    { //if(Task==REGRESSION){ ensrs converts time-series to converted to regression
	      fprintf(fp,"1          #0:time-series,1:function,3:ijcnn04,4:range-data,No.1\n");
	      fprintf(fp,"%d 0       #dimensionality k1 k2					\n",K);
	      fprintf(fp,"%s						       	        \n",fntrain[j]);
	      if(BAGGING==NoBAG || Task1==1){//fnpred
		fprintf(fp,"%s	#BAGGING==NoBAG || Task1==1\n",fntest[j]);
		//		printf("####check BAGGING=%d,Task1=%d\n",BAGGING,Task1);
	      }
	      else if(BAGGING==BAGwithVal){
		fprintf(fp,"%s 	#BAGwithVal\n",fn_bagging);//see below fn_givendata
		//		printf("####check BAGGING=%d,Task1=%d, fn_bagging=%s\n",BAGGING,Task1,fn_bagging);
	      }
	      else if(BAGGING==BAGwithoutVal){
		fprintf(fp,"%s #BAGGING==BAGwithoutVal  \n",fntest[j]);//bagging 20210527tag1:use fntest for pred 
		//fprintf(fp,"%s #BAGGING==BAGwithoutVal fn_givendata\n",fn_givendata);//bagging test original
	      }
	    ...
	    if(fn_net_exist){
	      if(nop==0) fprintf(stderr,"'%s' exists. Omit re-learning.\n",fn_net);
	      fprintf(fp,"nl         net-load		                \n");
	      fprintf(fp,"%s\n",fn_net);
	      if(r0!=0) fprintf(fp,"r0\n");
	      //	      cmdj[j][ii][0]=0;
	      //	      fprintf(stderr,"#cmdj[%d][%d]='%s,r0=%d'\n",j,ii,cmdj[j][ii],r0);
	    }
	    else{
	    ...
	    if(Task==TIMESERIES){
	      if(ssp==1) fprintf(fp,"ssp_ #single step prediction no-display with _.\n");//no effect 091027?
	    ...
	    //	    printf("#see %s.\n",fnparamj[j][ii]);
	    //	    printf("#BAGGING=%d,Nobag=%d,BAGwithVal=%d,Task=%d\n",BAGGING,NoBAG,BAGwithVal,Task);
...
//check	    if(1==1 || nop==0){
//check	      fprintf(stderr,"#check#param=%s BAGGING=%d\n",fnparamj[j][ii],BAGGING);
//check	      char cmd1[256];
//check	      sprintf(cmd1,"cat %s",fnparamj[j][ii]);
//check	      fprintf(stderr,"#cmd='%s'\n",cmd1);
//check	      system(cmd1);
//check	      fprintf(stderr,"#check#cmdj[%d][%d]='%s'\n",j,ii,cmdj[j][ii]);
//check	    }
	    //	      printf("#cmd:%s\n",cmdj[j][ii]);
	    //	    }//end of else{ if(fn_net_exist){
	  }//end of if(((fp=fopen(fn_pred,"r"))!=NULL)&&((fp1=fopen(fn_is,"r"))!=NULL)&&((fp0=fopen(fn_pred0,"r"))!=NULL)){
    ...
    for(j=0;j<nFolds;j++){//for all resampled-training datasets
      for(ii=0;ii<nens;ii++){//ensemble with different number of units
	//	fprintf(stderr,"#cmdj[%d][%d]=%s'\n",j,ii,cmdj[j][ii]);
	if(strncmp(cmdj[j][ii],"./can2 ",7)==0){	  //if(cmdj[j][ii][0]!=0){
	  ncmd++;
	  //	  fprintf(stderr,"#%d#",ncmd);
	}
	//	fprintf(stderr,"#cmdj[%d][%d]='%s'\n",j,ii,cmdj[j][ii]);
      }
    }
    //    fprintf(stdout,"#ncmd=%d",ncmd);

    if(ncmd>0){
      fprintf(stdout,"#start omp parallel using %d-cpu\'s for executing %d-'can2 ...',nFolds=%d,nens=%d",m_cpu,ncmd,nFolds,nens);fflush(stdout);
      if(m_cpu>ncmd) omp_set_num_threads(ncmd);

#pragma omp parallel for //#pragma omp parallel for openmp
    //////////////
    for(j=0;j<nFolds;j++){//for all resampled-training datasets
      for(ii=0;ii<nens;ii++){//ensemble with different number of units
	if(strncmp(cmdj[j][ii],"./can2 ",7)==0){	  //if(cmdj[j][ii][0]!=0){
	  ...
	  //sudo renice -n -20 `pidof ensrs`
	  //real5m51.512suser8m48.275ssys0m19.426s
	  //	  sprintf(cmd,"nice -n -20 ./%s",cmdj[j][ii]);//real5m22.392s user6m16.178s sys0m17.093s
	  //	  sprintf(cmd,"nice -n -10 ./%s",cmdj[j][ii]);//real5m22.392s user6m16.178s sys0m17.093s
	  //	  sprintf(cmd,"nice -20 %s",cmdj[j][ii]);//real5m22.392s user6m16.178s sys0m17.093s
	  //sprintf(cmd,"%s msg:thread%d",cmdj[j][ii],thread);
	  if(nop==0) fprintf(stdout,"#cmd(j%d,ii%d): %s\n",j,ii,cmd);
	  fprintf(stdout,"#cmd(j%d,ii%d): %s\n",j,ii,cmd);fflush(stdout);
	  //	  sprintf(cmd,"echo -n \"thread%d\";%s",thread,cmdj[j][ii]);//comment out
	  system(cmd);
	  //	  free(cmd);
	  //	  system(cmdj[j][ii]);
	}
	//	else{
	//	  fprintf(stdout,"#cmdj[%d][%d]=%s\n",j,ii,cmdj[j][ii]);
	//	}
      }
    }
    //////
    ...
    for(j=0;j<nFolds;j++){//for all resampled-training datasets
      for(ii=0;ii<nens;ii++){//ensemble with different number of units
	FILE* fp = fopen(fnnetj[j][ii],"r");
	if (fp == NULL) {
	  fprintf(stdout,"#no %s. Do '%s' again.",fnnetj[j][ii],cmdj[j][ii]);fflush(stdout);
	  system(cmdj[j][ii]);
	}
	else fclose(fp);
      }
    ...
    for(j=0;j<nFolds;j++){
      for(i=0;i<nens;i++) free(cmdj[j][i]);
      free(cmdj[j]);
    }
    free(cmdj);
    //////////////////////////////////////
	...
	//	fprintf(stdout,"#check \n");
      ...
      double Ypj[nFolds][n_pred];
      double ypj[nFolds][n_pred];
      int t;
      double xp[K+1];
      int hpred=n_pred;
      xp[K]=BIAS;
      for(t=0;t<total_steps;t++) y0[t]=moverange(Y0[t],ymin,ymax,ymin1,ymax1);
      for(t=-(K+tpD-1);t<0;t++)	yp[t]=moverange(Y0[t+tp0],ymin,ymax,ymin1,ymax1);
      for(t=0;t<n_pred;t++){
	double ypsum=0;
	double Ypsum=0;
	int nyp=0;
	...
	for(j=0;j<nFolds;j++){//for all resampled-training datasets
	  ...
	  { //if(fabs(Y-Y0[t+tp0])<1e-5){//to remove prediction with big error
	    ypsum+=y;
	    Ypsum+=Y;
	    Ypj[j][t]=Y;
	    ypj[j][t]=y;
	...
	fprintf(fp,"\n");
      ...
      fclose(fp);
      //      fprintf(stdout,"#prediction is saved in %s\n",fnpred);
      if(nop==0) fprintf(stdout,"#prediction is saved in %s\n",fnpred);
	...
	sprintf(&cmd[strlen(cmd)]," ;./pred2y_ts predict+.dat %s > /dev/null",fn_target);system(cmd);
	{
	  ...
	  sprintf(&msg[strlen(msg)],"[%d,%d](%.1fs) %.3e #[T,Tpinv](time) n%d,%d k%d N%d b%d a%g s%d nop%d m_cpu%d",T,Tpinv,mytimer_total(),mse,nTrainData,n,K,_nCells,nFolds,alpha,seed,nop,m_cpu);
	  //	  sprintf(&msg[strlen(msg)],"%s: msg2\n",msg);
	  printf("%s\n",msg);
	}
	...
	for(j=0;j<nFolds;j++){//for all resampled-training datasets
	  for(ii=0;ii<nens;ii++){//ensemble with different number of units
	    FILE *fp=fopen(fnpredj[j][ii],"r");
	    for(n=0;n<nGivenData;n++) {
	      int _idum;
	      if(nc[j*nGivenData+n]!=1){//not in Dtrain but in test 20210527tag1:use fntest for ypred 
		//{//use fn_givendata for pred
	        fgets(line1,LINESIZE,fp);
...
//    }//    if(Boost==NoBoost){//no boosting
	...
	}
	{
	  ...
	  if(fp==NULL) printf("#### predict+oob.dat open error\n");
	  else{
	    printf("#### predict+oob.dat open success\n");
	    for(n=0;n<nGivenData;n++){
