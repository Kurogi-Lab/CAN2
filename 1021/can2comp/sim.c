...
void exec_plot(DATA *givendata, DATA *result, char *title, char *type,NET *net) {
  //  char type[64];
  char dat[64], gpl[64], obj[64], fb[60];
...
//	    e*e,0.);
  ...
  //  fclose(fp);
  // 出力および誤差を計算
  if((fp=open_file(net->fnp,"w"))==NULL) return;  //write predict.dat
  for (t=n_train; t<n_total; t++) {
    calc_output(net, test->x[t],&test->y[t],&y,&(test->Y[t]));
    test->e[t] = (test->Y[t]-givendata->Y[t]);
...
void exec_ssp_test_NIPS04(NET *net, DATA *givendata, DATA *test) {
  ...
  ////////////////////////////
  // 出力および誤差を計算
  if((fp=open_file(net->fnp,"w"))==NULL) return;  //write predict.dat for NIPS04 and IJCNN06
  for (t=n_train; t<n_total; t++){
    calc_output(net, test->x[t],&test->y[t],&y,&(test->Y[t]));
    test->e[t] = (test->Y[t]-givendata->Y[t]);
#ifdef NEEDKYORI
...
void exec_ssp_train(NET *net, DATA *givendata, DATA *test) {
  ...
  // 出力および誤差を計算
  if((fp=open_file(net->fnp0,"w"))==NULL) return;  //prediction of training data
  for (t=0; t<n_train; t++) {
    //original data
    //    for(k=0;k<n_channels;k++) fprintf(fp,"%15.7e ",givendata->X[t][k]);
    //    fprintf(fp,"%15.7e ",givendata->Y[t]);
...
//    printf("2\n");
  ...
  if(givendata->data_class==TIME_SERIES){
    ...
    net->printf(strx5("N%dk%dw%2.1fvm%d:%d\n",net->n_cells,net->k,net->width,net->vmin,net->vmin2));
    int i=max_i_times;
    sprintf(&net->msg[strlen(net->msg)],"%d(%.3fs) %.3e %.3e #ep(time),MSEtr,MSEtst k%d n%d:%d N%d s%d",i,elapsedtime,MSEtrain[i],MSEssp[i],net->k1,givendata->n_train,givendata->n_test,net->n_cells,net->seed);
