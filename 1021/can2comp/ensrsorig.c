...
//    if((pdir=opendir(rdir))==NULL) {sprintf(cmd,"mkdir %s",rdir);system(cmd);} else closedir(pdir);
	...
	for(ii=0;ii<nens;ii++){
	  ...
	  if(((fp=fopen(fn_pred,"r"))!=NULL)&&((fp1=fopen(fn_is,"r"))!=NULL)&&((fp0=fopen(fn_pred0,"r"))!=NULL)){
	    ...
	    if(BAGGING==NoBAG || BAGGING==BAGwithVal){//20200108	    if(!BAGGING){
	    ...
	    fclose(fp);
            if(nop==0){
	      fprintf(stdout,"--- param.dat (BAGGING=%d)---\n",BAGGING);
	      system("cat param.dat");
	      fprintf(stdout,"--- param.dat ---\n");
	    }
	    if(fabs(BIAS-1.)>1e-10) {
              char _cmd[256];
	      //	      printf("#1fnp=%s\n#fn_pred=%s\n#fn_givendata=%s\n",fnp,fn_pred,fn_givendata);
	      sprintf(_cmd,"./can2 BIAS:%g LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d fnp:%s fnn:%s < param.dat",BIAS,LINESIZE,seed,Tpinv,nop,DISP,fnp,fn_net);
	      //sprintf(_cmd,"./can2 BIAS:%g LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d < param.dat",BIAS,LINESIZE,seed,Tpinv,nop,DISP);
	      ...
	      system(_cmd);//>>predict0.dat ??
              ...
              char _cmd[256];
	      sprintf(_cmd,"./can2 LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d fnp:%s fnn:%s < param.dat",LINESIZE,seed,Tpinv,nop,DISP,fnp,fn_net);
	      //sprintf(_cmd,"./can2 LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d< param.dat",LINESIZE,seed,Tpinv,nop,DISP);
	      system(_cmd);//--err
              net_printf(strx1("#%s\n",_cmd));
	    }
	    //	    printf("\n");
...
//20200108	    if(BAGGING==BAGwithVal){//for predict0.dat for bagging. training loss:  Lvar0, Lib, Lemp for the gn_givendata
//20200108	      fp=fopen("param.dat","w+");
//20200108	      fprintf(fp,"1          #0:time-series,1:function,3:ijcnn04,4:range-data,No.2\n");
//20200108	      //	      fprintf(fp,"1          #0:時系列,1:関数,3:ijcnn04,4:距離データ,No.2\n");
//20200108	      fprintf(fp,"%d 0     #dimensionality k1 k2				        \n",K);
//20200108	      //	      fprintf(fp,"%d 0     #次元 k1 k2				        \n",K);
//20200108	      //dummy
//20200108	      fprintf(fp,"%s 				\n",fn_bagging);//see below fn_givendata
//20200108	      //if(BAGGING==NoBAG)  fprintf(fp,"%s				        \n",fntest[j]);
//20200108	      //else if(BAGGING==BAGwithVal) fprintf(fp,"%s 				\n",fn_bagging);//see below fn_givendata
//20200108	      //else if(BAGGING==BAGwithoutVal) fprintf(fp,"%s 				\n",fn_givendata);//bagging test
//20200108	      fprintf(fp,"%s\n",fn_givendata);
//20200108	      //??090810	      fprintf(fp,"%s						       	        \n","/dev/null");//??
//20200108	      if(ymax-ymin<1e-20){
//20200108		fprintf(fp,"0 0 0 0   #normalization-of-output ymin0 ymax0 -> ymin1 ymax1	        \n"); 
//20200108		//		fprintf(fp,"0 0 0 0   出力の正規化 ymin0 ymax0 →ymin1 ymax1	        \n"); 
//20200108	      }
//20200108	      else{
//20200108		fprintf(fp,"%e %e %e %e    #normalization-of-output ymin0 ymax0 -> ymin1 ymax1	        \n",ymin,ymax,ymin1,ymax1); 
//20200108		//		fprintf(fp,"%e %e %e %e    出力の正規化 ymin0 ymax0 →ymin1 ymax1	        \n",ymin,ymax,ymin1,ymax1); 
//20200108		for(i=0;i<K;i++) fprintf(fp,"%e %e %e %e #input-%d-normalization xmin0 xmax0->xmin xmax\n",xmin[i],xmax[i],xmin1[i],xmax1[i],i);
//20200108		//for(i=0;i<K;i++) fprintf(fp,"%e %e %e %e #入力%dの正規化 xmin0 xmax0→xmin xmax\n",xmin[i],xmax[i],xmin1[i],xmax1[i],i);
//20200108		fprintf(fp,"%d %d %e #r1 r2 					        \n",r1,r2,r3);
//20200108	      }
//20200108	      //	      if(fabs(BIAS-1.)>1e-10) fprintf(fp,"BIAS:%e		        \n",BIAS);
//20200108	      fprintf(fp,"nl         net-load			                \n");
//20200108	      //	      fprintf(fp,"nl         ネットのロード			                \n");
//20200108	      fprintf(fp,"%s\n",fn_net);//	      fprintf(fp,"last.net\n");
//20200108	      fprintf(fp,"ssp0_      #prediction of training data -> predict0.dat    \n");//for Bayesian??
//20200108	      //	      fprintf(fp,"ssp0_      #訓練データの予測 > predict0.dat    \n");//for Bayesian??
//20200108	      if(lcom) fprintf(fp,"%s            \n",lcom);//for last command
//20200108	      fprintf(fp,"qu         \n");
//20200108	      fclose(fp);
//20200108              if(nop==0){
//20200108                printf("-- param.dat ---2\n");
//20200108                system("cat param.dat");
//20200108                printf("-- param.dat ---2\n");
//20200108              }
//20200108	      if(fabs(BIAS-1.)>1e-10) {
//20200108		char _cmd[256]; 
//20200108		sprintf(_cmd,"./can2 BIAS:%g LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d fnp:%s fnn:%s < param.dat",BIAS,LINESIZE,seed,Tpinv,nop,DISP,fnp,fn_net);
//20200108		//		sprintf(_cmd,"./can2 BIAS:%g LINESIZE:%d seed:%d Tpinv:%d nop:%d DISP:%d< param.dat",BIAS,LINESIZE,seed,Tpinv,nop,DISP);
//20200108		system(_cmd);//>>predict0.dat ??
//20200108	      }
//20200108	      else{
//20200108		char _cmd[256];
//20200108		sprintf(_cmd,"./can2 LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d fnp:%s fnn:%s < param.dat",LINESIZE,seed,Tpinv,nop,DISP,fnp,fn_net);
//20200108		//		sprintf(_cmd,"./can2 LINESIZE:%d seed:%d Tpinv:%d nop:%d DISP:%d < param.dat",LINESIZE,seed,Tpinv,nop,DISP);
//20200108		system(_cmd);//
//20200108		//		system("./can2 LINESIZE:%d < param.dat");//>>predict0.dat ??
//20200108	      }
//20200108	    }
#ifdef CHKTIME
...
//////////////////////////////////////
	...
	sprintf(&cmd[strlen(cmd)]," ;./pred2y_ts predict+.dat %s > /dev/null",fn_target);system(cmd);
	{
	  ...
	  fclose(fp);
	  sprintf(&msg[strlen(msg)],"%d-%d(%.1fs) %.3e #nbag(time) MSE n%d,%d N%d b%d a%g seed%d nop%d m_cpu%d",nFolds,nens,mytimer_total(),mse,nTrainData,n,_nCells,nFolds,alpha,seed,nop,1);
	  //	  sprintf(&msg[strlen(msg)],"%d-%d(%.3fs) %.3e %d #nbag(time) MSE ntest",nFolds,nens,mytimer_total(),mse,n);
	  printf("%s\n",msg);
	}
...
//    }//    if(Boost==NoBoost){//no boosting
	...
	fprintf(stderr,"#Ltst(n%.1f*%d)%.3e Lib(n%.1f*%d)%.3e H%.1f, nTest%.1f=%.1f=exp(-%.2f)*nGiven%dmethod:%sN%s.\n",_ntest,nGivenData0,Ltst,_ntrain,nGivenData1,Lib,H0,meannTestData,exp(-alpha)*nGivenData,alpha,nGivenData,argv[2],argv[3]);
      {
	sprintf(&msg[strlen(msg)],"%d-%d(%.3fs) %.3e #nFolds,nens(time) MSE, predict+.dat=%s ",nFolds,nens,mytimer_total(),Ltst,fn_target);
      }
...
#define GBBOOSTTEST 0
	...
	sprintf(&cmd[strlen(cmd)]," ;./pred2y_ts predict+.dat %s > /dev/null",fn_target);system(cmd);
	{
	  ...
	  fscanf(fp,"%lf%d",&mse,&n);
	  fclose(fp);
	  double elapsed=mytimer_total();
	  sprintf(&msg[strlen(msg)],"%d-%d(%.3fs) %.3e %d #nbag(time) MSE ntest",nFolds,nens,elapsed,mse,n);
	  printf("%s",msg);
	}
	//fprintf(stderr,"Prediction saved in '%s', Loss in'./loss+.dat'. Bagging with method=%s. \n",fn_target,argv[2]);
      }
...
//	  }
	...
	sprintf(&cmd[strlen(cmd)]," ;./pred2y_ts predict+.dat %s > /dev/null",fn_target);system(cmd);
	{
	  ...
	  fscanf(fp,"%lf%d",&mse,&n);
	  fclose(fp);
	  sprintf(&msg[strlen(msg)],"%d-%d(%.3fs) %.3e %d #nbag(time) MSE",nFolds,nens,mytimer_total(),mse,n);
	  printf("%s",msg);
	}
	//fprintf(stderr,"Prediction saved in '%s', Loss in'./loss+.dat'. Bagging with method=%s. \n",fn_target,argv[2]);
      }
