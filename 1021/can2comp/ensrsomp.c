...
#define TIMESERIES 0
#define REGRESSION 1
#define TIMESERIESbag 2
double tpEy=15;
int Task=1,tr0=0,tr1=0,tp0=0,tp1=0,tpD=1;
int Task1=0;//=BAGGINGTIMESERIES:task=REGRESSION, task1=TIMESERIES
double *Y0;
///////////////////////////
...
void init_walkeralias(int n,double *p)
{
  ...
  //  if(i>=0 && j>=0){
  //    fprintf(stderr,"?? strainge !? Walker alias (i=%d,j=%d)",i,j);
  //  }
  free(S);
  free(G);
}
...
//int setfntraintest(char **fntrain,char **fntest,unsigned int seed,char *fn_givendata,char **argv)
{
  ...
  if(method==-1){//direct validation no-bagging
    ...
    {//////////////////////////obtain training data, and separate them for CV
      ...
      FILE *fptrain_xy;
      FILE *fptest_xy;
      char line1[LINESIZE];//char line1[buffsize];
      int t;
      for(j=0;j<nFolds;j++){
      ...
      if(Task1==1){//
        ...
        for(t=0;;){
          ...
          if(feof(fp1)|| line1[0]==0) break;
          if(line1[0] == '#') continue;//20140303
          t++;
        }
        int total_steps=t;
        Y0=(double *) malloc(sizeof(double)*total_steps);
        rewind(fp1);
        for(t=0;;){
          fgets(line1,LINESIZE,fp1);
          if(feof(fp1)|| line1[0]==0) break;
          if(line1[0] == '#') continue;//20140303
          int nc=sscanf(line1, "%lf", &Y0[t]);
          if(nc<1) continue;
          t++;
        }
        int n_train0=tr1-tr0-K;
        int n_test0=1;
	//        double train0[n_train0][K+1];
	double **train0=(double **) malloc(sizeof(double*)*n_train0);
	for(t=0;t<n_train0;t++) train0[t]=(double *) malloc(sizeof(double)*(K+1));
        for(t=0;t<n_train0;t++){
          int t0=t+tr0;
          int i;
          for(i=0;i<K;i++) train0[t][i]=Y0[t0+K-i-1];
          train0[t][K]=Y0[t0+K+tpD-1];
        }
	//        double test0[n_test0][K+1];
	double **test0=(double **) malloc(sizeof(double*)*n_test0);
	for(t=0;t<n_test0;t++) test0[t]=(double *) malloc(sizeof(double)*(K+1));
        for(t=0;t<n_test0;t++){
          int t0=t+tp0;
          int i;
          for(i=0;i<K;i++) test0[t][i]=Y0[t0-i-1-tpD];
          test0[t][K]=Y0[t0];
        }
        nGivenData=n_train0;
        nTrainData=n_train0*alpha+0.5;
        nr=(int*) malloc(sizeof(int)*nTrainData*nFolds);
        nc=(int*) malloc(sizeof(int)*nGivenData*nFolds);
	double pbst[nGivenData*5];
	//	double *pbst=(double*) malloc(sizeof(double)*(nGivenData)*5);
        int n,nn;
	for(n=0;n<nGivenData;n++) pbst[n]=1./nGivenData;
	init_walkeralias(nGivenData,pbst);
        for(j=0;j<nFolds;j++){
          nTestData[j]=0;
        ...
        for(j=0;j<nFolds;j++){
          ...
          for(n=0;n<nTrainData;n++){
            int nn=nr[j*nTrainData+n];
            for(i=0;i<=K;i++) fprintf(fptrain_xy,"%.7e ",train0[nn][i]);
            fprintf(fptrain_xy,"\n");
          }
          fclose(fptrain_xy);
          fptest_xy=fopen(fntest[j],"a+");
          for(i=0;i<=K;i++) fprintf(fptest_xy,"%.7e ",test0[0][i]);
          fprintf(fptest_xy,"\n");
          fclose(fptest_xy);
        }
      }
...
int main(int argc,char **argv)
{
  ...
  char fn_target[fnsize]={"tmp/target.dat"};
  char fn_pred[fnsize];
  char fnp[fnsize];
...
#define FULLIREM2 4
  ...
  sprintf(fn_givendata,"%s",argv[1]);
  {
    ...
    ncol=sscanf(argv[2],"%d:%d:%lf:%d:%d",&method,&nFolds,&alpha,&seed,&m_cpu);
//    for(;*p!=0;p++) if(*p==':') ncol++;
...
//    else if(ncol==4) sscanf(argv[2],"%d:%d:%lf:%d:%d",&method,&nFolds,&alpha,&seed,&m_cpu);
//    else sscanf(argv[2],"%d",&method);//second element is fntest[0] 
  ...
  ///
  if(m_cpu<=0){
    m_cpu=omp_get_max_threads();
  }
  else{
    omp_set_num_threads(m_cpu);
  }
  ///
  if(method==MULTIFOLD){
    if(alpha<1) alpha=1;//?
...
//  }
  ...
  int ssp=0;
  //  int ttrainfrom,ttrainto,tpredfrom,tpredto;
  //  double tpEy=15;
...
//      }
	...
	sscanf(&argv[i][2],"%lf:%lf:%lf:%lf:%lf:%lf:%lf:%lf",&ymin,&ymax,&ymin1,&ymax1,&y0m,&y1m,&y0M,&y1M);
      ...
      for(;*p!=0;p++) if(*p=='-') nm++;
      if(nm==2){
	sscanf(&argv[i][2],"%d-%d:%d-%d:%d:%lf",&tr0,&tr1,&tp0,&tp1,&tpD,&tpEy);
      }
...
//      //      sscanf(&argv[i][4],"%d",&t_boost);
    ...
    //    else if(strncmp(argv[i],"t:",2)==0) sprintf(fn_target,"%s",&argv[i][2]);
      ...
      sscanf(&argv[i][2],"%d-%d:%d-%d:%d:%lf",&tr0,&tr1,&tp0,&tp1,&tpD,&tpEy);
      //      sscanf(&argv[i][2],"%d-%d:%d-%d:%d:%lf",&ttrainfrom,&ttrainto,&tpredfrom,&tpredto,&tpD,&tpEy);
      Task1=1;//bagging for time-series
      Task=REGRESSION;
      BAGGING=NoBAG;
    }
...
//      }
    ...
    else if(strncmp(argv[i],"m_cpu:",4)==0) sscanf(&argv[i][4],"%d",&nop);
...
#define ERR2BOOST
  ...
  if(BAGGING==BAGwithoutVal){
    yp0=(double*)malloc(sizeof(double)*nGivenData*nFolds*nens);//for BAGGING==2 & EmBoost
    yt=(double*)malloc(sizeof(double)*nGivenData);//for BAGGING==2
  }
  else yt=(double*)malloc(sizeof(double)*nValData);//for BAGGING==2
  {//
    int kk,ii;
    sprintf(rdir,"%s",rdir0);sprintf(cmd,"mkdir -p %s",rdir);system(cmd);
    sprintf(rdir,"%s/tmp",rdir0);sprintf(cmd,"mkdir -p %s",rdir);system(cmd);
    sprintf(rdir,"%s/net",rdir0);sprintf(cmd,"mkdir -p %s",rdir);system(cmd);
    ...
    char fnparamj[nFolds][nens][256]; //for parallel processing
    char fnpredj[nFolds][nens][256];
    char fnnetj[nFolds][nens][256];
    char cmdj[nFolds][nens][1024];
	...
	sprintf(fb0,"%s+s%dj%d",fnbody(fn_givendata,pr),seed,j);//argv[2]=method//argv[3]=nCells:nens
      ...
      //20200108      sprintf(fn_enspred,  "%s/tmp/%sk%dN%d-%d:%denspred.dat"  ,rdir0,fb,K,NN[0],NN[nens-1],NStep);
      //20200108#ifdef NEWp0
      //20200108      sprintf(fn_enspred0, "%s/tmp/%sk%dN%d-%d:%denspred0.dat" ,rdir0,fb0,K,NN[0],NN[nens-1],NStep);
      //20200108#else
      //20200108      sprintf(fn_enspred0, "%s/tmp/%sk%dN%d-%d:%denspred0.dat" ,rdir0,fb,K,NN[0],NN[nens-1],NStep);
      //20200108#endif
      //20200108      sprintf(fn_ensis,    "%s/tmp/%sk%dN%d-%d:%densis.dat"    ,rdir0,fb,K,NN[0],NN[nens-1],NStep);
      //20200108      //      sprintf(fn_targetens,"%s/tmp/%starget.dat"   ,rdir0,fb);
      //20200108      sprintf(fn_target,"%s/tmp/%starget.dat"   ,rdir0,fb);
      
      //20200108      if(((fp=fopen(fn_enspred,"r"))!=NULL)&&((fp1=fopen(fn_ensis,"r"))!=NULL)&&((fp0=fopen(fn_enspred0,"r"))!=NULL)){
      //20200108	fclose(fp);fclose(fp1);fclose(fp0);
      //20200108	fprintf(stderr,"'%s' exists. Omit recalc.\n",fn_enspred);
      //20200108	sprintf(cmd,"cp %s predict+.dat",fn_enspred);system(cmd);
      //20200108      }////use previous fn_pred 
      //20200108      else{//if no fn_enspred 
      {//20200108
	...
	for(ii=0;ii<nens;ii++){//ensemble with different number of units
	  _nCells=NN[ii];
	  //	  for(kk=0;kk<strlen(fb);kk++) if(fb[kk]=='/') fb[kk]='@';
	  ...
	  sprintf(fnp,"%s/tmp/%sk%dN%d",rdir0,fb,K,_nCells);
	  //          sprintf(fnisj[j][ii],"%s",fn_is);
	  if(((fp=fopen(fn_pred,"r"))!=NULL)&&((fp1=fopen(fn_is,"r"))!=NULL)&&((fp0=fopen(fn_pred0,"r"))!=NULL)){
	    ...
	    char fn_net[fnsize];
	    int fn_net_exist=0;
	    sprintf(fn_net,"%s/net/%s+m%da%gb%d:%ds%dj%dN%dK%d.net",rdir0,fnbody_givendata,method,alpha,t_boost,Boost,seed,j,_nCells,K);//
	    sprintf(fnnetj[j][ii],"%s",fn_net);
	    if((fp=fopen(fn_net,"r"))!=NULL){
	      fclose(fp);
	      fn_net_exist=1;
	    }
	    sprintf(fnparamj[j][ii],"tmp/param%03d-%03d.dat",j,ii);
	    sprintf(fnpredj[j][ii],"%s",fn_pred);
	    fp=fopen(fnparamj[j][ii],"w+");
	    { //if(Task==REGRESSION){
	      fprintf(fp,"1          #0:time-series,1:function,3:ijcnn04,4:range-data,No.1\n");
	      fprintf(fp,"%d 0       #dimensionality k1 k2					\n",K);
	      fprintf(fp,"%s						       	        \n",fntrain[j]);
	      if(BAGGING==NoBAG || Task1==1){
		fprintf(fp,"%s				        \n",fntest[j]);
	      }
		...
		if(Task1!=1){
		  ...
		  for(i=0;i<K;i++) fprintf(fp,"%e %e %e %e %e:%e:%e:%e  #x%d-normalization[ymin0:ymax0]->[ymin1:ymax1]\n",ymin,ymax,ymin1,ymax1,y0m,y1m,y0M,y1M,i); 
		}
...
//	    
	    ...
	    if(BAGGING==NoBAG || BAGGING==BAGwithVal){//??	    if(!BAGGING){
	      //if(BAGGING==BAGwithVal)
	    ...
	    fclose(fp);
            if(nop==0){
	      fprintf(stdout,"##param=%s BAGGING=%d\n",fnparamj[j][ii],BAGGING);
	      //	      system("cat param.dat");
	      //	      fprintf(stdout,"--- %s ---\n");
	    }
	    if(fabs(BIAS-1.)>1e-10){
	      //              char _cmd[80]; 
	      sprintf(cmdj[j][ii],"./can2 BIAS:%g LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d fnp:%s fnn:%s < %s",BIAS,LINESIZE,seed,Tpinv,nop,DISP,fnp,fnnetj[j][ii],fnparamj[j][ii]);
	      //              net_printf(strx1("#%s\n",_cmd));
	      //	      system(_cmd);//>>predict0.dat ??
	    }
	    else{
	      //              char _cmd[80]; 
	      sprintf(cmdj[j][ii],"./can2 LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d fnp:%s fnn:%s < %s",LINESIZE,seed,Tpinv,nop,DISP,fnp,fnnetj[j][ii],fnparamj[j][ii]);
	      //check
//	      fprintf(stdout,"#check cmd:%s\n",cmdj[j][ii]);
//	      fprintf(stdout,"#check \n");
	    ...
	    //	    }
	    //20200108	    if(method!=-1 || Task==TIMESERIES){
	    //20200108	      //	      fp=fopen("is.dat","w+");
	    //20200108	      //	      fp=fopen(fnisj[j][ii],"w+");
	    //20200108	      if(BAGGING==NoBAG){
	    //20200108		FILE *fpmse=fopen("mse0.dat","r");fscanf(fpmse,"%lf",&msetrain[j]);fclose(fpmse);
	    //20200108		fprintf(fp,"%d %15.7e %d %15.7e %d %d %d %15.7e %15.7e\n",0,0.,nTrainData,msetrain[j],_nCells,_nCells,nTestData[j],0.,0.);
	    //20200108	      }
	    //20200108	      else if(BAGGING==BAGwithVal){
	    //20200108		fprintf(fp,"%d %15.7e %d %15.7e %d %d %d %15.7e %15.7e\n",0,0.,nTrainData,0.,_nCells,_nCells,nValData,0.,0.);
	    //20200108	      }
	    //20200108	      else if(BAGGING==BAGwithoutVal){
	    ...
	    ////////////////////
	    ////////////////////
	    //20200108	    if(1==0 && BAGGING==BAGwithVal){//for predict0.dat for bagging. training loss:  Lvar0, Lib, Lemp for the gn_givendata
	    //20200108	      //	      fp=fopen("param.dat","w+");
	    //20200108	      fp=fopen(fnparamj2[j],"w+");
	    //20200108	      fprintf(fp,"1          #0:time-series,1:function,3:ijcnn04,4:range-data,No.2\n");
	    //20200108	      fprintf(fp,"%d 0     #dimensionality k1 k2\n",K);
	    //20200108	      fprintf(fp,"%s \n",fn_bagging);//see below fn_givendata
	    //20200108	      fprintf(fp,"%s \n",fn_givendata);//any?
	    //20200108	      if(ymax-ymin<1e-20){
	    //20200108		fprintf(fp,"0 0 0 0   #normalization-of-output ymin0 ymax0 -> ymin1 ymax1	        \n"); 
	    //20200108	      }
	    //20200108	      else{
	    //20200108		fprintf(fp,"%e %e %e %e    #normalization-of-output ymin0 ymax0 -> ymin1 ymax1	        \n",ymin,ymax,ymin1,ymax1); 
	    //20200108		for(i=0;i<K;i++) fprintf(fp,"%e %e %e %e #input-%d-normalization xmin0 xmax0->xmin xmax\n",xmin[i],xmax[i],xmin1[i],xmax1[i],i);
	    //20200108		fprintf(fp,"%d %d %e #r1 r2 					        \n",r1,r2,r3);
	    //20200108	      }
	    //20200108	      fprintf(fp,"nl         net-load \n");
	    //20200108	      fprintf(fp,"%s\n",fn_net);//	      fprintf(fp,"last.net\n");
	    //20200108	      fprintf(fp,"ssp0_      #prediction of training data -> predict0.dat    \n");//for Bayesian??
	    //20200108	      if(lcom) fprintf(fp,"%s            \n",lcom);//for last command
	    //20200108	      fprintf(fp,"qu         \n");
	    //20200108	      fclose(fp);
	    //20200108              if(nop==0){
	    //20200108		fprintf(stdout,"##param=%s\n",fnparamj[j]);
	    //20200108		//                printf("--- param.dat ---\n");
	    //20200108//                system("cat param.dat");
	    //20200108//                printf("--- param.dat ---\n");
	    //20200108              }
	    //20200108	      if(fabs(BIAS-1.)>1e-10) {
	    //20200108
	    //20200108		sprintf(cmdj[j],"./can2 BIAS:%g LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d fnp:%s fnn:%s fnis:%s < %s",BIAS,LINESIZE,seed,Tpinv,nop,DISP,fnpredj[j],fnnetj[j],fnisj[j],fnparamj[j]);
	    //20200108		//		char _cmd[40]; 
	    //20200108		//sprintf(_cmd    ,"./can2 BIAS:%g LINESIZE:%d seed:%d Tpinv:%d nop:%d DISP:%d< param.dat",BIAS,LINESIZE,seed,Tpinv,nop,DISP);system(_cmd);//>>predict0.dat ??
	    //20200108	      }
	    //20200108	      else{
	    //20200108		sprintf(cmdj[j],"./can2 LINESIZE:%d seed:%u Tpinv:%d nop:%d DISP:%d fnp:%s fnn:%s fnis:%s < %s",LINESIZE,seed,Tpinv,nop,DISP,fnpredj[j],fnnetj[j],fnisj[j],fnparamj[j]);		//		system("./can2 LINESIZE:%d < param.dat");//>>predict0.dat ??
	    //20200108		//		char _cmd[40]; 
	    //20200108		// 		sprintf(_cmd   ,"./can2 LINESIZE:%d seed:%d Tpinv:%d nop:%d DISP:%d< param.dat",LINESIZE,seed,Tpinv,nop,DISP);system(_cmd);//
	    //20200108	      }
	    //20200108	    }//if(BAGGING==BAGwithVal){
	    //20200108	    if(BAGGING!=BAGwithoutVal) {sprintf(cmd,"cp predict0.dat %s",fn_pred0);system(cmd);}//???prediction of training data
	    //20200108	    else {sprintf(cmd,"cp /dev/null %s",fn_pred0);system(cmd);}
	  ...
	  /////////////////////////////////////////////////////////////////////
	  //20200108	  {
	  //20200108	    int lc=strlen(cmd1),la=strlen(fn_pred);//char *oldcmd1=cmd1;
	  //20200108	    if(cmd1size<lc+la){
	  //20200108	      cmd1size=lc+10*la;
	  //20200108	      cmd1=realloc(cmd1,cmd1size);
	  //20200108	      //	      if(cmd1!=oldcmd1) {int j; for(j=0;j<lc;j++) cmd1[j]=oldcmd1[j];}
	  //20200108	    }
	  //20200108	    sprintf(&cmd1[lc]," %s",fn_pred);//combine meanpred 
	  //20200108	  }
	  //20200108	  if(BAGGING==NoBAG){//	if(ERR==MSE){
	  //20200108	    double _mseis,_mse,_msetrain,_msetrainis; 
	  //20200108	    int _nCells1;
	  //20200108	    fp=fopen(fn_is,"r");
	  //20200108	    fscanf(fp,"%d %lf %d %lf %d %d %d %lf %lf",&is[j],&_mseis,&n_msetrain[j],&_msetrain,&n_cells2[j],&_nCells1,&n_mse[j],&_mse,&_msetrainis);
	  //20200108	    msetrain[j]+=(_msetrain*n_msetrain[j]);//rational
	  //20200108	    n_msetrainsum+=n_msetrain[j];
	  //20200108	    fclose(fp);
	  //20200108	  }
//?	  printf("cmdj[%d][%d]=%s\n",j,ii,cmdj[j][ii]);
//?	  sprintf(cmd,"%s",cmdj[j][ii]);
//?	  if(cmd[0]!=0){
//?	    system(cmd);
//?	  }
	  int lc=strlen(cmd2),la=strlen(fn_pred);//char *oldcmd2=cmd2;
	  if(cmd2size<lc+la){
	    cmd2size=lc+10*la;
	    cmd2=realloc(cmd2,cmd2size);
	    //	if(cmd2!=oldcmd2) {int j; for(j=0;j<lc;j++) cmd2[j]=oldcmd2[j];}
	  }
	  sprintf(&cmd2[lc]," %s",fn_pred);//combine meanpred 
	}//closing  for(ii=0;ii<nens;ii++){
	//20200108	{//ensemble,mse.dat,loss+.dat for nFolds
	//20200108	  msetrain[j]/=(n_msetrainsum);//Modified at 060517
	//20200108	  n_msetrain[j]=n_msetrainsum;//modified at 080707??
	//20200108	  net_printf(strx1("Doing '%s'\n",cmd1));system(cmd1);//exec meanpred for ensemble
	//20200108	  sprintf(cmd,"cp predict+.dat %s",fn_enspred);system(cmd);
	//20200108	  sprintf(cmd,"cat %s | awk '{print $%d}' > %s",fntest[j],K+1,fn_target);system(cmd);
	//20200108	  sprintf(&cmd[strlen(cmd)]," ;./pred2y_ts predict+.dat %s > /dev/null",fn_target);system(cmd);
	//20200108	  fp=fopen("mse.dat","r"); fscanf(fp,"%lf",&mse[j]);fclose(fp);
	//20200108	  fp=fopen("loss+.dat","r"); fscanf(fp,"%lf",&Lhat[j]);fclose(fp);
	//20200108	}
      }//closing else{//if no fn_enspred 
      //if((BAGGING==BAGwithoutVal||BAGGING==NoBAG) && Boost!=GbBoost){//for training(empirical) and prediction(generalization) error
//20200127      if(BAGGING==BAGwithoutVal && Boost!=GbBoost){//for training(empirical) and prediction(generalization) error
//20200127	//if(BAGGING==BAGwithoutVal){//for training(empirical) and prediction(generalization) error
//20200127	//if(BAGGING==BAGwithoutVal && Boost==EmBoost){//for training(empirical) and prediction(generalization) error
//20200127	//Boost==EmBoost && Boost == NoBoost for manipulate training and test predictions
//20200127	int _idum;
//20200127	fp=fopen(fn_pred,"r");
//20200127	for(n=0;n<nGivenData;n++) {
//20200127	  fgets(line1,LINESIZE,fp);
//20200127	  sscanf(line1,"%lf%d%lf",&yp0[(ii*nFolds+j)*nGivenData+n],&_idum,&yt[n]);
//20200127	  //	  sscanf(line1,"%lf%d%lf",&yp0[j*nGivenData+n],&_idum,&yt[n]);
//20200127	}
//20200127	fclose(fp);
//20200127      }
//20200108      {//if(Bayes>0 && BAGGING==BAGwithVal){
//20200108	{//if(bayes[j]==j){
//20200108	  sprintf(&cmd2[lc]," %s",fn_pred);
//20200108	  sprintf(fn_bayes,"%s",fn_pred);
//20200108	  //	  sprintf(&cmd2[lc]," %s",fn_enspred);
//20200108	  //	  sprintf(fn_bayes,"%s",fn_enspred);
//20200108	  bused++;
//20200108	}
//20200108	//20200108 	else{
//20200108	//20200108 	  if(strlen(fn_bayes)>=1) sprintf(&cmd2[lc]," %s",fn_bayes);
//20200108	//20200108 	  //	  else n_bayesopt++;
//20200108	//20200108 	}
//20200108	//if(bayes[0]==j) sprintf(fn_bayesopt,"%s",fn_enspred);
//20200108      }
//20200108      else sprintf(&cmd2[lc]," %s",fn_pred);//combine meanpred 
...
//	  sprintf(cmd,"%s msg:thred%d",cmdj[j][ii],thred);
//	  fprintf(stdout,"echo -n \"thred%d\";%s",thred,cmdj[j][ii]);
//	  //	  system(cmd);
//	  //	  system(cmdj[j][ii]);
//	}
//      }
//    }
	...
	if(cmdj[j][ii][0]!=0){
	  ...
	  //	  sprintf(cmd,"echo -n \"thred%d\";%s",thred,cmdj[j][ii]);
	  system(cmd);
	  //	  system(cmdj[j][ii]);
	}
      }
    ...
    //////////////////////////////////////
    //////add process of cmd1 for boosting predictions with different (nens-) number of units 
    //////////////////////////////////////
#ifdef CHKTIME
    ...
    if(Task1==1){
      NET net[nFolds];
      for(j=0;j<nFolds;j++){//for all resampled-training datasets
	//	char *fn_net=fnnetj[j][0];
	net[j].Vinit=net[j].init=0;
	//	fprintf(stdout,"#check fn_net:%s\n",fnnetj[j][0]);
	net_load(&net[j],fnnetj[j][0]);
	//	fprintf(stdout,"#check \n");
      }
      int n_pred=tp1-tp0;
      double _yp[n_pred+K+tpD-1];
      double _Yp[n_pred+K+tpD-1];
      double _Ypj[nFolds][n_pred+K+tpD-1];
      double *yp=&_yp[K+tpD-1];
      double *Yp=&_Yp[K+tpD-1];
      //      double **Ypj=(double**)malloc(sizeof(double*)*nFolds);
      double *Ypj[nFolds];
      for(j=0;j<nFolds;j++){
	Ypj[j]=(double *)&_Ypj[j][K+tpD-1];
      }
      int t;
      double xp[K+1];
      int hpred=n_pred;
      xp[K]=BIAS;
      for(t=-(K+tpD-1);t<0;t++){
	yp[t]=moverange(Y0[t+tp0],ymin,ymax,ymin1,ymax1);
      }
      for(t=0;t<n_pred;t++){
	double ypsum=0;
	double Ypsum=0;
	for(j=0;j<nFolds;j++){//for all resampled-training datasets
	  double yr,y,Y;
	  for(i=0;i<K;i++) xp[i]=yp[t-tpD-i];
	  calc_output(&net[j],xp,&yr,&y,&Y);
	  ypsum+=y;
	  Ypsum+=Y;
	  Ypj[j][t]=Y;
	}
	...
	Yp[t]=Ypsum/nFolds;
      ...
      for(t=0;t<n_pred;t++){
	if(fabs(Yp[t]-Y0[t+tp0])>tpEy){
	  hpred=t;
	  break;
	}
      }
      char fnpred[256];
      sprintf(fnpred,"tmp/pred%d-%d.dat",tp0,tp1);
      FILE *fp=fopen(fnpred,"w");
      for(t=0;t<n_pred;t++){
	//	fprintf(fp,"%7e %d %.7e\n",Yp[t],t+tp0,Y0[t+tp0]);
	fprintf(fp,"%7e %d %.7e",Yp[t],t+tp0,Y0[t+tp0]);
	//	fprintf(stdout,"%7e %d %.7e\n",Yp[t],t+tp0,Y0[t+tp0]);
	for(j=0;j<nFolds;j++) fprintf(fp," %.7e",Ypj[j][t]);
	fprintf(fp,"\n");
      }
      fclose(fp);
      sprintf(&msg[strlen(msg)],"[%d,%d](%.1fs) #{T,Tinv] k%d N%d b%d(nens%d) a%g seed%d nop%d m_cpu%d %d-%d:%d-%d:%gH%d",T,Tpinv,mytimer_total(),K,_nCells,nFolds,nens,alpha,seed,nop,m_cpu,tr0,tr1,tp0,tp1,tpEy,hpred);
      printf("%s\n",msg);
      return(0);
    }
	...
	{
	  double mse;int n;
	  FILE *fp=fopen("mse.dat","r");
	  fscanf(fp,"%lf%d",&mse,&n);
	  fclose(fp);
	  //	  printf("%d-%d(%.3fs) %.3e %d #nbag(time) MSE ntest",nFolds,nens,mytimer_total(),mse,n);
	  sprintf(&msg[strlen(msg)],"[%d,%d](%.1fs) k%d N%d b%d a%g s%d nop%d m_cpu%d",T,Tpinv,mytimer_total(),K,_nCells,nFolds,alpha,seed,nop,m_cpu);
	  printf("%s",msg);
	}
	...
	Ltst=Lib=Lemp=0;
	if(Bayes>0){
	  random_seed(BayesSeed);//??not work?
//#if RAND == ZMTRAND
...
//	    else LambdaL=0;
	    ...
	    b1=jm;
	    for(j=0;j<nFolds;j++){
	      if(Lb[j]<Lb[b1] || random_unif1()<exp((Lb[b1]-Lb[j])*LambdaL)){
		if(Lb[j]<1e20) {
		  b1=j;nbayes++;
		}
	      }
		...
		b1=j;
	      ...
	      //	      else if(Lb[j]<Lb[j-1] || random_unif1()<exp((Lb[j-1]-Lb[j])*LambdaL)){
	      else if(Lb[j]<Lb[b1] || random_unif1()<exp((Lb[b1]-Lb[j])*LambdaL)){
		if(Lb[j]<1e20){
		  b1=j; nbayes++;
		}
	      }
	    ...
	    for(j=1;j<nFolds;j++){
	      if(Lb[j]<Lb[j-1] || random_unif1()<exp((Lb[j-1]-Lb[j])*LambdaL))	b1=j;
	      fprintf(fpb,"%d %d %.5e %.5e\n",b1,j,Lb[j],Lb[b1]);
	      bayes[j]=b1;
	    }
	    fclose(fpb);
...
#define metropolisNEW
	    ...
	    for(j=0;j<nFolds;j++){
	      ...
	      if(p>0 || random_unif1()<exp(p)){
		//if(Lb[j]<Lb[b1] || random_unif1()<exp((Lb[b1]-Lb[j])*LambdaL+(Sb[b1]-Sb[j])*LambdaS)){
		if(Lb[j]<1e20) {
		  b1=j;nbayes++;
		}
	      }
...
#define LambdaLOrig
	    ...
	    int b1; b1=j=0;fprintf(fpb,"%d %d %.5e %.5e\n",b1,j,Lb[j],Lb[b1]); bayes[j]=j;
	    for(j=1;j<nFolds;j++){
	      if(Lb[j]<Lb[j-1] || random_unif1()<exp((Lb[j-1]-Lb[j])*LambdaL)) b1=j;//metropolis
	      fprintf(fpb,"%d %d %.5e %.5e\n",b1,j,Lb[j],Lb[b1]);
	      bayes[j]=b1;
	    }
	    fclose(fpb);
