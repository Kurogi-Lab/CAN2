...
#          wp=2. # width of pole
          ...
          nrx2=nr*2
          qcb=np.zeros((na,nrx2), dtype=np.float32)
          for n in range(N):
            Pn=P[n]
            for i in range(k):
...
#                  ia=int(na*(a+pi)/pix2) #atan2 in [-pi,pi]
                  qc[ia,ir]+=c
                  q1[ia,ir]+=1
                  qcb[ia,ir]+=Pn[i,2]
...
#              G1  = F1 #==cv2.GaussianBlur(F1,(1,1),1)
              Gcb = cv2.GaussianBlur(Fcb,(args.Ksize,args.Ksize),args.Ksigma)
              Qc=cv2.GaussianBlur(qc,(args.Ksize,args.Ksize),args.Ksigma)
#              Q1= q1
              Qcb=cv2.GaussianBlur(qcb,(args.Ksize,args.Ksize),args.Ksigma)
...
#              Ytl.append('{:.1f}'.format(y0m))
  ...
  #            fig.add_subplot(gs[2,0],xticks=Xtq,xticklabels=Xtlq,yticks=Ytb,yticklabels=Ytlb)
              mx=qcDisp.max()
              plt.imshow(qcDisp/mx,cmap=cmap);
  ...
  #
              ...
              mx=q1Disp.max()
              plt.imshow(q1Disp/mx,cmap=cmap);
  ...
  #
              ...
              _min=qcbDisp.min()
              mx=max(abs(_max),abs(_min))
              plt.imshow(qcbDisp/mx,cmap=cmap);#
...
#            print('#check2 s[{}]:{} d:{} l:{} n:{}'.format(i_s,s,d,l,n));
          ...
          fn='{}/{}-rPc.dat'.format(dr,fnB)#  
          df=pd.DataFrame(Gc.reshape((1,-1)))
          df.to_csv(fn,index=False,sep=' ',header=None)
          fn='{}/{}-rPcb.dat'.format(dr,fnB)#  
          df=pd.DataFrame(Gcb.reshape((1,-1)))
          df.to_csv(fn,index=False,sep=' ',header=None)
          fn='{}/{}-rP1.dat'.format(dr,fnB)#  
          df=pd.DataFrame(G1.reshape((1,-1)))
          df.to_csv(fn,index=False,sep=' ',header=None)

          fn='{}/{}-pPc.dat'.format(dr,fnB)#  
          df=pd.DataFrame(Qc.reshape((1,-1)))
          df.to_csv(fn,index=False,sep=' ',header=None)
          fn='{}/{}-pPcb.dat'.format(dr,fnB)#  
          df=pd.DataFrame(Qcb.reshape((1,-1)))
          df.to_csv(fn,index=False,sep=' ',header=None)
          fn='{}/{}-pP1.dat'.format(dr,fnB)#  
          df=pd.DataFrame(Q1.reshape((1,-1)))
          df.to_csv(fn,index=False,sep=' ',header=None)
          print('#{}-[rPc|rPcb|rP1|pPc|pPcb|Ppc].dat created (it:{})'.format(fnB,it))
