...
#                myshell('cp {}/{}-{}{}-R{}.dat {}/{}'.format(dn,s,d,l,r,dm,sm))
  ...
  #              import pdb;pdb.set_trace(); #for debug
              ...
              print(cmd)
              cmd='cp tmp/M.dat {}/s{}R{}d{}l{}-M.dat'.format(df,s,R,d,l)
              myshell(cmd)
...
#              print(cmd);
              myshell(cmd)
              cmd='cp tmp/M.dat {}/M-s{}R{}.dat'.format(dq,s,R)
              myshell(cmd);
...
#              M=np.array(pd.read_csv(fnM,delim_whitespace=True,dtype=np.float32,header=None))
#              N,k1=M.shape
#              fnmac='tmp/M.mac'
#              fpmac=open(fnmac,'a')
#              for n in range(N):
...
#              cmd='echo {}bach("M2xyC.mac");{}|maxima'.format("'",k,n,"'")
              cmd='maxima < M2xyC.mac' #read tmp/M.dat produce tmp/xyC.dat
              myshell(cmd);
              cmd='cp tmp/M.dat {}/M-s{}R{}.dat'.format(dq,s,R)
              myshell(cmd);
              cmd='cp tmp/xyC.dat {}/syC-s{}R{}.dat'.format(dq,s,R)
