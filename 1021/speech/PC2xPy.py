...
#####            if i_fd>=3 and nr>=4: #remove both edges
#####              if i_fd<=4: #remove 2 edges
#####                F1=F1[:,1:-1]
#####              else: #remove 4 edges
#####                F1=np.concatenate((F1[:,1:nr-2],F1[:,nr+1:-1]), axis = 1)
#####              df=pd.DataFrame(F1.reshape((1,-1)))
#####              fnr='{}/{}-{}F{}x{}G{}x{}m{}_.dat'.format(dr,fnB,fd,nr,na,nG,sG,mel)
#####              df.to_csv(fnr,index=False,sep=' ',header=None)
#              import pdb;pdb.set_trace(); #for debug 20191105
#            print('#check')
###          if nG ==1:
