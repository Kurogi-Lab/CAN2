...
#define cmdsize 10240
  ...
  //
  {
    ...
    if((pdir=opendir("./result-ensrs2ge"))==NULL) system("mkdir ./result-ensrs2ge"); else closedir(pdir);
    if((pdir=opendir("./result-ensrs2ge/tmp"))==NULL) system("mkdir ./result-ensrs2ge/tmp"); else closedir(pdir);
  }
  ///
...
//  double *skwN=(double *)malloc(sizeof(double)*(_nN));
  ...
  if(DiffMode==1){//             dy
	...
	//
	//      sprintf(fnpredlong,"result-ensrs2ge/tmp/%s+%d-%d:%d-%dk%d-dy-predict+.dat",fnbody(argv[1],pr),ttrainfrom,ttrainto-m*mpd,tpredfrom-m*mpd,tpredto,k);
	sprintf(fnpredlong,"result-ensrs2ge/tmp/%s+%d-%d:%d-%dk%dN%da%gb%d-dy-predict+.dat",fnbody(argv[1],pr),ttrainfrom,tmtrainto,tmpredfrom,tpredto,k,N,rs_alpha,rs_ens);
...
//	}
	...
	sprintf(fnpredlong,"result-ensrs2ge/tmp/%s+%d-%d:%d-%dk%dN%da%gb%dT%d-y-predict+.dat",fnbody(argv[1],pr),ttrainfrom,ttrainto-m*mpd,tpredfrom-m*mpd,tpredto,k,N,rs_alpha,rs_ens,T);
	if((fp=fopen(fnpredlong,"r"))!=NULL && *pupdate=='0'){
	  fprintf(stderr,"#Read from '%s'\n",fnpredlong);
	  for(i=tmpredfrom;i<tpredto;i++){
	    fgets(buff,buffsize,fp);
