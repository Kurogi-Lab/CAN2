...
int main(int argc,char **argv)
{
  ...
  char fn_target[fnsize+25];
  char fn_pred[fnsize+25];
  char fn_pred0[fnsize+25];
  char fn_is[fnsize+25];
  char fn_enspred[fnsize+25];
  char fn_enspred0[fnsize+25];
  char fn_ensis[fnsize+25];
  char fb[fnsize];
#define NEWp0
#ifdef NEWp0
  char fb0[fnsize];
...
//    if((pdir=opendir(rdir))==NULL) {sprintf(cmd,"mkdir %s",rdir);system(cmd);} else closedir(pdir);
	...
	sprintf(cmd,"cp %s predict+.dat",fn_enspred);system(cmd);
      ...
      else{//else of if(((fp=fopen(fn_enspred...
	...
	for(ii=0;ii<nens;ii++){
	  ...
	  if(((fp=fopen(fn_pred,"r"))!=NULL)&&((fp1=fopen(fn_is,"r"))!=NULL)&&((fp0=fopen(fn_pred0,"r"))!=NULL)){
		...
		fprintf(fp,"%d %d %e #r1 r2 					        \n",r1,r2,r3);
	      ...
	      if(fabs(BIAS-1.)>1e-10) {
		char _cmd[40+50]; sprintf(_cmd,"./can2 BIAS:%g LINESIZE:%d seed:%d Tpinv:%d nop:%d DISP:%d< param.dat",BIAS,LINESIZE,seed,Tpinv,nop,DISP);system(_cmd);//>>predict0.dat ??
	      }
	      else{
		char _cmd[40+25]; sprintf(_cmd,"./can2 LINESIZE:%d seed:%d Tpinv:%d nop:%d DISP:%d< param.dat",LINESIZE,seed,Tpinv,nop,DISP);system(_cmd);//
	...
	{//ensemble,mse.dat,loss+.dat for nFolds
      ...
      }//closing 'else{//else of if(((fp=fopen(fn_enspred...'
      //}//for(j=0;j<nFolds;j++){
      //}//main level 1
	...
	for(n=0;n<nGivenData;n++){
	  ...
	  for(j=0;j<nFolds;j++){
	    if(nc[j*nGivenData+n]!=1){//not in Dtrain
	      ntest[n]++;
	      _ntest++;
	      yptest[n]+=yp0[j*nGivenData+n];//not in Dtrain
	    }
	    else{//in Dtrain
	      ntrain[n]++;
	      _ntrain++;
