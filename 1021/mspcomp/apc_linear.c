...
#endif
#ifdef MSPC
#define AP_ny 4//4//4//4//4//4//4//
#define AP_nu 4//4//4//4//4//4//4//
int CAN2_T=20;
...
/*    AP_z[0]=1e-4;   /*最小二乗の定数項の重み  */
  ...
  //  AP_u_max=linear.dXmax/5.;  /*入力最大値の設定 */
  //  AP_u_min=-linear.dXmax/5.;  /*入力最大値の設定 */ //061227kuwa
  AP_kS1=AP_tS1/AP_tS;  
  /*********************/
  set_AP_r();/*目標温度の設定・変更*/
  /*********************/
...
//    if(it>=1){//check memory allocation
//      double *dum=(double*)malloc(sizeof(double)*10000);
//      fprintf(stderr,"#memory-check1 dum=%p AP_du[1]=%g j%d\n",dum,AP_du[1],j);
//      free(dum);
//    }
...
//    if(it>=1){//check memory allocation
//      double *dum=(double*)malloc(sizeof(double)*10000);
//      fprintf(stderr,"#memory-check3 dum=%p AP_du[1]=%g\n",dum,AP_du[1]);
//      free(dum);
//    }
//	  if(1==0){//check
...
//	  }
	  ...
	  for(i=1;i<=num_tems;i++){//
	    int k;for(k=0;k<=CAN2_k;k++) tem[0].net[0].cell[0].M[0][k]=tem[i_mse_min].net[0].cell[0].M[0][k];
	  }
	  AP_i=0;//
	  if(1==0){//check
	    fprintf(stdout,"time%g <M>=",time);
...
//    if(it>=1){//check memory allocation
//      double *dum=(double*)malloc(sizeof(double)*10000);
//      fprintf(stderr,"#memory-check4 dum=%p AP_du[1]=%g\n",dum,AP_du[1]);
//      free(dum);
//    }
...
/************************************************************************/
{
  ...
  int hi;//080131yuno
  {
	...
	sscanf(&argv[i][3],"%lf",&_linear_m);
      }
      else if(strncmp(argv[i],"cr:",3)==0){
	sscanf(&argv[i][3],"%lf",&_linear_r);
      }
      else if(strncmp(argv[i],"cdXmax:",7)==0){
	sscanf(&argv[i][7],"%lf",&_linear_dXmax);
      }
      else if(strncmp(argv[i],"cC:",3)==0){
	sscanf(&argv[i][3],"%lf",&_linear_C);
      }
//      else if(strncmp(argv[i],"cFmax:",6)==0){
//	sscanf(&argv[i][6],"%lf",&_linear_Fmax);
//      }
      else if(strncmp(argv[i],"ky:",3)==0){
	sscanf(&argv[i][3],"%lf",&_rr_kyoyou);//subst to rr_kyoyou in linear2sub.c 
      }
