...
if [ "$dataclean" = "yes" ]; then 
	 ...
	 if [ "$model" = "nonlinear" ]
	 then
	     cmd="./emulate_crane2 it:$controlTrainingIterations:$itblock r:$r cr:$cr cm:$cm cC:$cC ck:$ck umax:$umax ur:$ur uT:$uT tt:$tt kxt:1 method:12:${unit}:${b}:$a:0 DISP:$DISP listSS:1 T:$T N2s:$N2s LAMBDA:0.01 ky:$kyoyou RI:$RIstring"
	     echo $cmd; time $cmd;
#		time ./emulate_crane2 it:$controlTrainingIterations:$itblock r:$r cr:$cr cm:$cm cC:$cC ck:$ck umax:$umax tt:$tt kxt:1 method:12:${unit}:${b}:$a:0 DISP:$DISP listSS:1 T:$T N2s:$N2s LAMBDA:0.01 ky:$kyoyou RI:$RIstring
	 ...
	 if [ "$model" = "linear" ]
	 then
	     cmd="./emulate_crane4 it:$controlTrainingIterations:$itblock r:$r cr:$cr cm:$cm cC:$cC ck:$ck umax:$umax  ur:$ur uT:$uT tt:$tt kxt:1 method:12:${unit}:${b}:$a:0 DISP:$DISP listSS:1 T:$T N2s:$N2s LAMBDA:0.01 ky:$kyoyou RI:$RIstring"
	     echo $cmd; time $cmd;
