...
date
echo "exec for cr=$crRange cm=$cmRange ur0=$Sur0 N=$N model=$model"
cat /dev/null > listSS.dat

for cr in $crRange; do for cm in $cmRange; do
  for ur0 in $Sur0; do
#  for ur0 in 0.5 0.6 0.7 0.8 0.9 1.0; do
#  for ur0 in 0.7 0.8 0.9; do
if [ "$dataclean" = "yes" ]; then 
	 ...
	 if [ "$model" = "nonlinear" ]
	 then
	     cmd="./emulate_crane2 it:$controlTrainingIterations:$itblock r:$r cr:$cr cm:$cm cC:$cC ck:$ck umax:$umax ur0:$ur0 tt:$tt kxt:1 method:12:${unit}:${b}:$a:0 DISP:$DISP listSS:1 T:$T N2s:$N2s LAMBDA:0.01 ky:$kyoyou RI:$RIstring"
	     echo $cmd; time $cmd;
#		time ./emulate_crane2 it:$controlTrainingIterations:$itblock r:$r cr:$cr cm:$cm cC:$cC ck:$ck umax:$umax tt:$tt kxt:1 method:12:${unit}:${b}:$a:0 DISP:$DISP listSS:1 T:$T N2s:$N2s LAMBDA:0.01 ky:$kyoyou RI:$RIstring
	 ...
	 if [ "$model" = "linear" ]
	 then
	     cmd="./emulate_crane4 it:$controlTrainingIterations:$itblock r:$r cr:$cr cm:$cm cC:$cC ck:$ck umax:$umax  ur0:$ur0 tt:$tt kxt:1 method:12:${unit}:${b}:$a:0 DISP:$DISP listSS:1 T:$T N2s:$N2s LAMBDA:0.01 ky:$kyoyou RI:$RIstring"
	     echo $cmd; time $cmd;
