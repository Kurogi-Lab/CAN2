...
#define Ne 50//mseの保存時間=MSE Ne=Ne*AP_tS/AP_TS1 for swiching CAN2
...
typedef struct {
  int firstcall;
  CELL *cell;//cell[100];//配列の中身はAP_NC_MAX(50)だった
  int  n_cells;
} NET;
...
int net_load(NET *net,char *fname,int a_n,int units){
  ...
  fprintf(stderr,"net_k=%d=%d+%d,units=%d,a_n=%d\n",net_k,AP_nu,AP_ny,units,a_n);
  for(i=0;i<units;i++){
    fprintf(stderr,"i=%d,net_k=%d=%d+%d,units=%d,a_n=%d\n",i,net_k,AP_nu,AP_ny,units,a_n);
...
//  double AP_k_start=AP_kS1*(AP_ny+AP_IS);/*090331kuro*/
  ...
  if(AP_k_hajime>=AP_k_start){/*090331kuro*/
	...
	for(i=1;i<=num_nets;i++){//check for ICONIP2010
	  ...
	  ndy2=0;
	  if(AP_n_kk<Ne){//mseの保存時間??
	    for(hi=1;hi<=AP_n_kk;hi++) ndy2+=mse[i][hi];
	    n_err[i]=ndy2;
	  }	  
	  else{
	    for(hi=AP_n_kk-Ne;hi<=AP_n_kk;hi++) ndy2+=mse[i][hi];//Ne*0.01
	    n_err[i]=ndy2;
	  }
	...
	boost_i=1;	
	for(m=1;m<=(boosting_times*n_AP_NC);m++){//Boosting?? no boosting for boosting_times=1 ?
...
//20101213	if((i%(resample_num*boosting_times))==0) bag_i++;
//20101213      }
...
//20101213	for(m=1;m<=(boosting_times*n_AP_NC+n_AP_NC);m++){
...
//20101213	}  
//20101213	boost_i=1;	
//20101213	for(m=1;m<=(boosting_times*n_AP_NC);m++){//Boosting
//20101213	  net[net_num+(boosting_times*n_AP_NC)+boost_i].cell[AP_i].M[0][i]+=(boosting_c_t[m]*net[net_num+m].cell[AP_i].M[0][i]);
//20101213	  if((m%boosting_times)==0){
//20101213	    net[net_num+(boosting_times*n_AP_NC)+boost_i].cell[AP_i].M[0][i]/=c_t_sum;
//20101213	    net[0].cell[AP_i].M[0][i] += net[net_num+(boosting_times*n_AP_NC)+boost_i].cell[AP_i].M[0][i];
//20101213	    boost_i++;
//20101213	  }
//20101213	}	  
...
//20101213	  //Qの計算
//20101213	  bag_i=1;
//20101213	  for(m=1;m<=(boosting_times*n_AP_NC+n_AP_NC);m++){//Bagging
...
//20101213	  boost_i=1;	
//20101213	  for(m=1;m<=(boosting_times*n_AP_NC);m++){//Boosting
//20101213	    net[net_num+(boosting_times*n_AP_NC)+boost_i].cell[AP_i].Q[i][j]+=(boosting_c_t[m]*net[net_num+m].cell[AP_i].Q[i][j]);
//20101213	    if((m%boosting_times)==0){
//20101213	      net[net_num+(boosting_times*n_AP_NC)+boost_i].cell[AP_i].Q[i][j]/=c_t_sum;
//20101213	      net[0].cell[AP_i].Q[i][j] += net[net_num+(boosting_times*n_AP_NC)+boost_i].cell[AP_i].Q[i][j];
//20101213	      boost_i++;
//20101213	    }
//20101213	  } 	    
...
//      }
  ...
  //ens:ensemble(1):bagging_flag(?):boosting_flag(1):boosting_times
  //num_nets(1):net_f
  ...
  if(method==0){//original Question&Answer sequence.
	...
	//    scanf("%s",&net_n[hi]);
      ...
      //080318 koshi
      if((uni_flag==1)&&(nuse_flag==0)){//merge
  ...
  ////////////////////////////////////////////////////////////
  int num_nets1=num_nets+1;
  net=(NET*)malloc(sizeof(NET)*num_nets1);
  int i;
  for(i=0;i<num_nets1;i++){
    net[i].cell=(CELL*)malloc(sizeof(CELL)*AP_NC);
    int j;
    fprintf(stderr,"AP_NN=%d,AP_NC=%d\n",AP_NN,AP_NC);
    for(j=0;j<AP_NC;j++){
      net[i].cell[j].w=(FLOAT*)malloc(sizeof(FLOAT)*(AP_nu+AP_ny));
      net[i].cell[j].M=(FLOAT**)malloc(sizeof(FLOAT*)*1);
      net[i].cell[j].M[0]=(FLOAT*)malloc(sizeof(FLOAT)*AP_NN);
