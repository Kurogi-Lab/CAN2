...
def pdDataFrameGpu(data,gpu):
#202103230 cuda is not necessary
#  return(pd.DataFrame(data))
  if gpu<0:
    return(pd.DataFrame(data))
  else:
    return(pd.DataFrame(data.get()))
#    from chainer import cuda
#    return(pd.DataFrame(cuda.to_cpu(data)))
...
### load_data() in my_function.c
  ...
  parser.add_argument('-fn', type=str, default="",
                      help='fntrain,fntest,fnpred')
#                      help='fntrain,fntest,IDex :IDex:ID of execution for parallel processing')
  parser.add_argument('-t', type=str, default='',
                      help='null for regression, t:tr0-tr1:tp0-tp1:tpD:Ey for |tpD|-step ahead prediction with recursive tpD>0, non-recursive tpD<0 ')
#  parser.add_argument('-t', type=str, default='',
...
### normalize_data(DATA *givendata, NET *net)
 ...
 ###init_net() in my_plinn.c
  ...
  parser.add_argument('-fns', type=str, default='',
                      help='file name to save net')
  parser.add_argument('-fnl', type=str, default='',
                      help='file name to load net')
...
#                      help='width')
##########exec_sim
  parser.add_argument('-ex', type=str, default="1,0.05,0.7,100,5,50,350", 
...
#    import chainer.links as L
    my_misc.myshell('cat /dev/null>use_gpu.py;sleep 0')
  else:
    my_misc.myshell('rm -f use_gpu.*;sleep 0');
...
#      import pdb;pdb.set_trace(); #for debug 
  ...
  #
    ...
    net=my_plinn.net_load(args.fnl)
    net['fntrain']=fntrain
    net['fntest']=fntest
    net['fnpred']=fnpred
    net['t']=args.t
...
#
    ...
    fnlst.append(fn)
    fn='tmp/M.csv'
    df=pdDataFrameGpu(net['am']['M'].reshape((net['n_cells'],net['n_channels']+1)),args.gpu)
    df.to_csv(fn,index=False,sep=' ',header=None, float_format='%.7e')
    fnlst.append(fn)
    fn='tmp/v.csv'
