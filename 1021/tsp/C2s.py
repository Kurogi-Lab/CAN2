...
#  parser.add_argument('-FD', default='s', type=str,   help='s or S: s for standard se(spectrum envelopw), S for se with pole weight=1 ')
  parser.add_argument('-FD', default='s', type=str,   help='s,S,sR,SR,sC,SC: s for standard se(spectrum envelopw), S for se with pole weight=1 ')
...
#    Fq=[(f.split('/')[-1])[:-4] for f in glob.glob(fnq)] #file list
    Fq=[(f.split('/')[-1])[:-6] for f in glob.glob(fnq)] #file list
#    import pdb;pdb.set_trace(); #for debaug
    for s in Sa:
      for d in Da:
        for l in La:
#          import pdb;pdb.set_trace(); #for debaug
