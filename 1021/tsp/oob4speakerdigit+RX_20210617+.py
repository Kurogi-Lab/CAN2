...
#    fntrain_m='{}/{}{}-{}.dat'.format(dr,FDext,fbtrain,m)
    ...
    if args.AF !='':
      AF=(args.AF).split(':')
      import pdb;pdb.set_trace(); #for debug
      Ftrain=np.array(pd.read_csv('{}/{}{}.dat'.format(args.dr,AF[0],fbtrain),delim_whitespace=True,dtype=np.float64,header=None))
      for i in range(0,len(AF)):
        Ftrain1=np.array(pd.read_csv('{}/{}{}.dat'.format(args.dr,AF[i],fbtrain),delim_whitespace=True,dtype=np.float64,header=None))
