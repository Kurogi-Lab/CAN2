...
#oob4speakerdigit+ sp:fhs:fms:mkk:mko:mmt:mnh:mym tx:zero:ichi:ni:san:yon:go:roku:nana:hachi:kyu ntxi:10 k:36 mbas:2:300:1.6:1 dir:$dira s:-1 N:40
  ...
  parser.add_argument('-dr', default='../../12voicedata_ueki_all_Ck30l20', type=str,   help='folder involving r')
...
#  parser.add_argument('-dms', default='../../12voicedata_ueki_all_ms', type=str,   help='folder involving mixed')
...
#                k=args.k;N=args.N;t0=0;t1=10000;rsa=args.rsa #t1=400 will be changed to the number of all data by poledistribv2-.c 20210410
              ...
              elif method[0]='SPE'>0: #Spectral Envelope
                (k,l,nDisp)=method[1:]
...
#                cmd='python lpcspectrumenvelope.py -yin {} -k {} -l {} -mp_th {} -fout {} -nDisp {} -t {}'.format(dmsm,args.k,args.l,args.mp_th,fnout,nDisp,args.t)
                ...
                if args.fmt=='f':
                  fnbin='tmp/tmp.flt'
                  fnmfcc='tmp/mfcc.bin'
                  cmd='python f2b.py -fmt {} -yin {} -yout {}'.format('f',dmsm,fnbin)
                  cmd='{};python f2b.py -fmt {} -yin {} -yout {};cat {}|frame -l {} -p {} | mfcc -l {} -f {} -m {} -n {} -a {} -E > {}'.format(cmd,'f',fnbin,fl,p,fl,fs,m,n,a,fnmfcc)
                  cmd='{};python print_mfcc.py > {}'.format('f',cmd,fnout)
                  
                  #python f2b.py -fmt 'f' -yin ../../12voicedata_ueki_all_n/mym-kyu10.dat -yout tmp/tmp.flt
                elif args.fmt=='h':
                  fnbin='tmp/tmp.sht'
                  cmd='python f2b.py -fmt {} -yin {} -yout {};x2x +sf < {} | frame -l {} -p {} | mfcc -l {} -f {} -m {} -n {} -a {} {} > {}'.format('h',dmsm,fnbin,fnbin,fl,p,fl,fs,m,n,a,E)
#                cmd='x2x +sf < {} | frame -l {} -p {} | mfcc -l {} -f {} -m {} -n {} -a {} {} > {}'.format(fnbin,fl,p,fl,fs,m,n,a,E)#short
#                x2x +sf < $fnshort | frame -l $fl -p $p | mfcc -l $fl -f $fs -m $m -n $n -a $a $E > data.mfc
              print(cmd);
#              import pdb;pdb.set_trace(); #for debug
              mylib.myshell(cmd)
              #poledistribv2- 
