...
#  fnrX=args.dir+'/speaker-verif-result-X-R{}{}.dat'.format(args.R,args.FD)
  ...
  if method[0][:4]=='MFCC':
    ...
    E_MFCC=int(E)
    FD=''
    FDext='MFCCl{}m{}n{}E{}'.format(l,m,n,1) #    FDext='{}F{}'.format(FD,nx)
#    import pdb;pdb.set_trace(); #for debug
  elif method[0][:6]=='LPC+SE':
...
#    import pdb;pdb.set_trace(); #for debug
    FtrainAF=np.array(pd.read_csv('{}{}.dat'.format(AF[0],fbtrain),delim_whitespace=True,dtype=np.float64,header=None))[:,:-1]
    for i in range(1,len(AF)):
      FtrainAF1=np.array(pd.read_csv('{}{}.dat'.format(AF[i],fbtrain),delim_whitespace=True,dtype=np.float64,header=None))[:,:-1]
#      import pdb;pdb.set_trace(); #for debug
...
#    import pdb;pdb.set_trace(); #for debug
    fntrain_m='{}/{}{}.dat'.format(dr,FDext,fbtrain,m) #same x,different y for different m?
...
#      Ftrain1=pd.read_csv(fntrain,delim_whitespace=True,dtype=np.str,header=None) #current real target
      Ftrain1=np.array(pd.read_csv(fntrain,delim_whitespace=True,dtype=np.float64,header=None)) #current real target
