...
//      }
    ...
    //    fprintf(fpw,"#%d sm\n",sm);
    {
      ...
      //      TP/=TPFN;FN/=(TPFN+ZERO);FP/=(FPTN+ZERO);TN/=(FPTN+ZERO);
      ...
      if(nsd==1){//speaker identification
        ...
        sprintf(com,"%.3f %.3f %.3f %.3f %.3g = %.0f %.0f %.0f %.0f %.0f %d #TP,TN,FP,FN,ER=...#n,sm%d ",TPR,TNR,FPR,FNR,ERR,TP,TN,FP,FN,ER,n,sm)
      }
      else if(nsd==2){//digit identification
        txm=m;
        //        sprintf(com,"%.3f %.3f %.3f %.3f %.3e %d %d #TP,TN,FP,FN,ERR,n,txm%d",TP,TN,FP,FN,ER,n,txm,txm);
        sprintf(com,"%.3f %.3f %.3f %.3f %.3g = %.0f %.0f %.0f %.0f %.0f %d #TP,TN,FP,FN,ER=...#n,dm%d ",TPR,TNR,FPR,FNR,ERR,TP,TN,FP,FN,ER,n,sm)
      }
      else if(nsd==3){
        sm=m/nD;txm=m%nD;
        sprintf(com,"%.3f %.3f %.3f %.3f %.3g = %.0f %.0f %.0f %.0f %.0f %d #TP,TN,FP,FN,ER=...#n,sm%d,dm%d",TPR,TNR,FPR,FNR,ERR,TP,TN,FP,FN,ER,n,sm,txm)
