...
#include <math.h>
#ifndef CRANESUB
#define CRANESUB 1 //または#define CRANESUB 2 
#endif
#define EMULATE
...
//#endif
...
#else //crane2
      if((crane.x>AP_r+2.*rr_kyoyou)&& (crane.dX>0) && (AP_u>0)) AP_u=-0.1*AP_u_max;//??
      if((crane.x<AP_r-2.*rr_kyoyou)&& (crane.dX<0) && (AP_u<0)) AP_u= 0.1*AP_u_max;//??
      //      if((AP_y>AP_r+2.*rr_kyoyou)&& (crane.dX>0) && (AP_u>0)) AP_u=-0.1*AP_u_max;//??
      //      if((AP_y<AP_r-2.*rr_kyoyou)&& (crane.dX<0) && (AP_u<0)) AP_u= 0.1*AP_u_max;//??
      //if((AP_y>AP_r+2.*rr_kyoyou)&& (crane.dX>0) && (AP_u>0)) AP_u*=-0.1;//??
  ...
  /////////////////////
//#if CRANESUB == 2
//  AP_y=PV1 = crane.X;// 台車の位置を使う→振動しない？
//  //    SMV=AP_u=crane.F;//加速度，速度制限があると入力AP_uと実際の操作量crane.Fが違うので非線形
//#endif
...
#if CRANESUB == 2
  AP_y=PV1 = crane.X;// 台車の位置を使う→振動しない？
  //  AP_y=PV1 = (crane.x+crane.X)/2.;// 台車の位置を使う→振動しない？
  //    SMV=AP_u=crane.F;//加速度，速度制限があると入力AP_uと実際の操作量crane.Fが違うので非線形
#endif
  //??what for yy
//?  double yy=0;
...
//?  //
  ...
  if(kk>=totaltime/AP_tS){
    ...
    if(listSS){
      ...
      fprintf(fp,"%.1f %2.0f %2d #ST[s] OS[mm] i for cr%gcm%gN%dny%dnu%diti%dIS%dr%gT%d\n",seiteijikoku-starttime,overshoot*1000.,it,_crane_r,_crane_m,AP_NC,AP_ny,AP_nu,_itblock,AP_IS,AP_r,CAN2_T);
      fclose(fp);
    }
    ////090608 yuno
    //#ifdef MSPC
  ...
  //081023 koshi, 090622 yuno
   ...
   if(DISP !=0){
     ...
     fprintf(fp,"#\n");
     fprintf(fp,"plot \"_y.dat\" using 1:13 t \"r\", \"_y.dat\" using 1:($2/10) t \"F/10\", \"_y.dat\" using 1:($9) t \"X\", \"_y.dat\" using 1:3 t \"x\"\n");
  ...
  jumpflag: //ok here
   ...
   if(ensemble==0){//
	 ...
	 for(n=0;n<=num_nets;n++){
	   for(i=0;i<AP_NC;i++){//AP_NC=sum uni_AP_NC[]
