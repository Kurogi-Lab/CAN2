...
EOF
  ...
  for pz in poles; do #  for pz in poles zeros; do #error with zero @20191231
    ...
    for nmL in $nL; do #for nmL in 1 2 3 4 5; do #M=1,2 for ST, 3,4 for OS
      for n in $nU; do #for n in `seq 1 ${Na[${nmL}]}` ; do # echo ${Na[@]} = 0 10 10 11 10 2
        let nM=$n
        C=0;
        echo "" >> ${fplt}
        echo "unset multiplot" >> $fplt
        echo "set term postscript eps enhanced color; set output \"gpc${rf}-$pz-mX-mL${mL1[${nmL}]}${obj[${nmL}]}.eps\";" >> $fplt
...
cmd="gv -scale=5.0 gpc-poles-mX-mL${mL1[${nL}]}${obj[${nL}]}.eps"
${cmd}&
echo "Do $cmd."
