...
ny=4;nu=1;sed -e s/"AP_ny "/"AP_ny ${ny}\/\/"/ -e s/"AP_nu "/"AP_nu ${nu}\/\/"/ $f1 > $f0 ; make CC=icc   ##1st trial
#ny=4;nu=4;sed -e s/"AP_ny "/"AP_ny ${ny}\/\/"/ -e s/"AP_nu "/"AP_nu ${nu}\/\/"/ $f1 > $f0 ; make CC=icc   ;##2nd trial
#ny=7;nu=4;sed -e s/"AP_ny "/"AP_ny ${ny}\/\/"/ -e s/"AP_nu "/"AP_nu ${nu}\/\/"/ $f1 > $f0 ; make CC=icc   ;##2nd trial
cat /dev/null > listSS.txt
###
...
#do once to here
for cm in 10 40 70 100; do #No.3
...
for cm3 in 10N 40N 70N 100; do export M=100 #Exp.No.3
#for cm in 10N 40N 70N 100; do export M=100   #Exp.No.3
#for cm in 75N 80N 85N 90N 95N; do
export q="cr2cm${cm3}";
cmd1=`cat listSS.dat|awk 'BEGIN {vm=1e9;l=0;q=ENVIRON["q"];} {l++;p=substr($8,1,8);if($1>0 && p==q){v=$2+$1*0.01; if(v<vm) {vm=v;i=l}}} END {printf("head -%d listSS.dat",i)}'`;echo -n "#BestOS+0.01ST: ";$cmd1 | tail -1;
cmd2=`cat listSS.dat|awk 'BEGIN {vm=1e9;l=0;q=ENVIRON["q"];M=ENVIRON["M"];} {l++;p=substr($8,1,8);if($1>0 && p==q && $2<M) {v=$1+0.01*$2; if(v<vm) {vm=v;i=l}}} END {printf("head %d listSS.dat",-i)}'`;echo -n "#BestST+0.01OS: ";$cmd2 | tail -1
done
