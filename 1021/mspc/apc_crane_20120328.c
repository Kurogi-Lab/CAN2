...
#ifdef EMULATE
//#define AP_TS1 0.5 //Virtual Sampling Period 仮想サンプリング周期
#define AP_TS1 0.5 //Virtual Sampling Period 仮想サンプリング周期
GLOBAL	WORD AP_N2=10/AP_TS1; // soso 
...
#endif
#ifdef MSPC
#define AP_ny 4//4//4//4//
#define AP_nu 4//4//4//4//
int CAN2_T=20;
...
GLOBAL float bag_alpha=0.7;//1;
GLOBAL int boosting_flag=0;
GLOBAL int boosting_steps=1;
GLOBAL double *boosting_c_t,c_t_sum;
//GLOBAL double boosting_c_t[100],c_t_sum;
GLOBAL int resample_num=1;
int net_num;//number of net for boosting?net_num=boosting_steps*resample_num*n_AP_NC;?
//090622 yuno
...
GLOBAL FLOAT n_err[100];//to save in y_matrix.dat
GLOBAL FLOAT ndy=0;
GLOBAL FLOAT ndy2=0;
...
//#define save_time 110//mseの保存時間=MSE Ne=save_time*AP_tS/AP_TS1 for swiching CAN2
//#define save_time 60//mseの保存時間=MSE Ne=save_time*AP_tS/AP_TS1 for swiching CAN2
#define save_time 50//mseの保存時間=MSE Ne=save_time*AP_tS/AP_TS1 for swiching CAN2
...
int net_load(NET *net,char *fname,int a_n,int units){
  ...
  if(fname==NULL){
    ...
    if((fp = fopen(fn, "r"))==NULL){
      fprintf(stderr,"##1# netfile:'%s' open err.\n",fn);
      exit(-1);
      //      return(1);
    }
  }
...
{//modify tomisaki 2005/11/9
  ...
  else{/*2007/02/18/koshi*///obtain_new!=0
    for(i=0;i<u_n;i++){
...
//  double AP_k_start=AP_kS1*(AP_ny+AP_IS);/*090331kuro*/
  ...
  if(AP_k_hajime>=AP_k_start){/*090331kuro*/
	...
	for(i=1;i<=num_nets;i++){//check for ICONIP2010
	  ...
	  //ndy2=energy_n[i];
	  mse[i][AP_n_kk]=(energy_n1[i]*energy_n1[i]);//energy of i-th net at AP_n_kk(time)
	  ndy2=0;
	  if(AP_n_kk<save_time){//mseの保存時間??
	    for(hi=1;hi<=AP_n_kk;hi++) ndy2+=mse[i][hi];
	    n_err[i]=ndy2;
	  }	  
	  else{
	    for(hi=AP_n_kk-save_time;hi<=AP_n_kk;hi++) ndy2+=mse[i][hi];//save_time*0.01
	    n_err[i]=ndy2;
	  }
	...
	AP_i=obtain_AP_i(AP_z,AP_y,net,0,AP_NC);// 荷重ベクトルによるユニットの選択
	...
	if((i%(resample_num*boosting_steps))==0) bag_i++;
      }     
      AP_i=0;//ユニット1のため
      //連想行列の平均を取る
      for(i=0;i<AP_NN;i++){
	...
	for(m=1;m<=(boosting_steps*n_AP_NC+n_AP_NC);m++){
	...
	boost_i=1;	
	for(m=1;m<=(boosting_steps*n_AP_NC);m++){//Boosting?? no boosting for boosting_steps=1 ?
	  net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].M[0][i]+=(boosting_c_t[m]*net[net_num+m].cell[AP_i].M[0][i]);
	  if((m%boosting_steps)==0){
	    net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].M[0][i]/=c_t_sum;
	    net[0].cell[AP_i].M[0][i] += net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].M[0][i];
	    boost_i++;
	  }
	}
	...
	for(j=0;j<AP_NN;j++){
	  ...
	  //Qの計算
	  bag_i=1;
	  for(m=1;m<=(boosting_steps*n_AP_NC+n_AP_NC);m++){//Bagging
	  ...
	  boost_i=1;	
	  for(m=1;m<=(boosting_steps*n_AP_NC);m++){//Boosting
	    net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].Q[i][j]+=(boosting_c_t[m]*net[net_num+m].cell[AP_i].Q[i][j]);
	    if((m%boosting_steps)==0){
	      net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].Q[i][j]/=c_t_sum;
	      net[0].cell[AP_i].Q[i][j] += net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].Q[i][j];
	      boost_i++;
	    }
	  } 
...
//20101213	if((i%(resample_num*boosting_steps))==0) bag_i++;
//20101213      }
...
//20101213	for(m=1;m<=(boosting_steps*n_AP_NC+n_AP_NC);m++){
...
//20101213	}  
//20101213	boost_i=1;	
//20101213	for(m=1;m<=(boosting_steps*n_AP_NC);m++){//Boosting
//20101213	  net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].M[0][i]+=(boosting_c_t[m]*net[net_num+m].cell[AP_i].M[0][i]);
//20101213	  if((m%boosting_steps)==0){
//20101213	    net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].M[0][i]/=c_t_sum;
//20101213	    net[0].cell[AP_i].M[0][i] += net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].M[0][i];
//20101213	    boost_i++;
//20101213	  }
//20101213	}	  
...
//20101213	  //Qの計算
//20101213	  bag_i=1;
//20101213	  for(m=1;m<=(boosting_steps*n_AP_NC+n_AP_NC);m++){//Bagging
...
//20101213	  boost_i=1;	
//20101213	  for(m=1;m<=(boosting_steps*n_AP_NC);m++){//Boosting
//20101213	    net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].Q[i][j]+=(boosting_c_t[m]*net[net_num+m].cell[AP_i].Q[i][j]);
//20101213	    if((m%boosting_steps)==0){
//20101213	      net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].Q[i][j]/=c_t_sum;
//20101213	      net[0].cell[AP_i].Q[i][j] += net[net_num+(boosting_steps*n_AP_NC)+boost_i].cell[AP_i].Q[i][j];
//20101213	      boost_i++;
//20101213	    }
//20101213	  } 	    
...
/************************************************************************/
{
  ...
  int hi;//080131yuno
  {
	...
	sscanf(&argv[i][3],"%d:%d",&iteration,&_itblock);
      ...
      else if(strncmp(argv[i],"listSS:",7)==0) listSS=&argv[i][7];
...
//      }
	...
	if(method==1){//single CAN2
	  ...
	  for(;;) if(*(p++)==':') break;
	  sscanf(p,"%d:%d:%f:%d",&AP_NC,&bag_b,&bag_alpha,&bag_seed);//rsa:$a:$bs:$b
	  ensemble=1;
	  uni_flag=nuse_flag=0;//??default?
	}
  ...
  net=(NET*)malloc(sizeof(NET)*(num_nets+1));
  int i;  for(i=0;i<=num_nets;i++) net[i].cell=(CELL*)malloc(sizeof(CELL)*AP_NC);
  ...
  //ens:ensemble(1):bagging_flag(?):boosting_flag(1):boosting_steps
  //num_nets(1):net_f
  ...
  if(method==0){//original Question&Answer sequence.
	...
	fgets(buff,buffsize,stdin);sscanf(buff,"%d",&boosting_steps);
	//scanf("%d",&boosting_steps);
	if(boosting_steps<1) boosting_steps = 1;
      }
    }
    net_num=boosting_steps*resample_num*n_AP_NC;
    //yuno071203
  ...
  boosting_c_t=(double*)malloc(boosting_steps*n_AP_NC*sizeof(double));//?
  //  boosting_c_t=(double*)malloc(boosting_steps*sizeof(double)*n_AP_NC);
  for(iii=0;iii<boosting_steps*n_AP_NC;iii++){
    boosting_c_t[iii]=1; //netの重み初期化
  }
  c_t_sum=boosting_steps;   
  printf("f_AP_NC=%d,e_AP_NC=%d,n_AP_NC=%d,AP_NC=%d,ensemble=%d,resample_num=%d,bag_alpha=%f,boosting_steps=%d,net_num=%d\n",f_AP_NC,e_AP_NC,n_AP_NC,AP_NC,ensemble,resample_num,bag_alpha,boosting_steps,net_num);/*060322kuwa*/ 
  net->firstcall=1;
