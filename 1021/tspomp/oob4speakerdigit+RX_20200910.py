...
def vrv2_f(f): # 
  ...
  if verif=="s":
    ...
    if len(fl)==6: #e.g. fhs-go1-mmt-nana10-R0.8-p.dat -> fms-ni2-R0.8-mmt-san4-pP1.dat
      r=fl[4][1:]
      v2=fl[2] #-> fhs-go1-mmt-nana10-R0.8-p.dat -> fms-ni2-R0.8-mmt-san4-pP1.dat 
    ...
    if len(fl)==6: #e.g. fhs-go1-mmt-nana10-R0.8-pP1F4x24G3x0.5m0.dat 
      r=fl[4][1:]
...
#oob4speakerdigit+ sp:fhs:fms:mkk:mko:mmt:mnh:mym tx:zero:ichi:ni:san:yon:go:roku:nana:hachi:kyu ntxi:10 k:36 mbas:2:300:1.6:1 dir:$dira s:-1 N:40
  ...
  parser.add_argument('-FD', default='pP1', type=str, choices=['pP1','pPc','pPcb','rP1','rPc','rPcb','s','S','sR','SR'],help='Features of data')
...
#    fntrain='train_xy'
#    fntest='test_xy'
    fntrain='train_xy_{}'.format(m)
    fntest='test_xy_{}'.format(m)
...
#    import pdb;pdb.set_trace(); #for debug
    ...
    while True: #??
      ...
      if os.path.exists(fnpred):
        break
      print '#Error on creating {}!'.format(fnpred)
      mylib.myshell('cat ./tmp/ensrs.log')
      print '#Error on creating {}? Try again!'.format(fnpred)
#      time.sleep(1)
...
#    import pdb;pdb.set_trace(); #for debug
  ...
  fprX.close()
  #  import pdb;pdb.set_trace(); #for debug
  cmd='cat {}|awk \'BEGIN{{n=0;TP=TN=FP=FN=ERR=0}}{{n+=$13;TP+=$7;TN+=$8;FP+=$9;FN+=$10;ERR+=$11}}END{{RC=TP/(TP+FN);PR=TP/(TP+FP);F=2*RC*PR/(RC+PR);printf("#TP%.3f TN%.3f FP%.3f FN%.3f ER%.3f RC%.3f PR%.3f F%.3f k{} {}lx{} {}\\n",TP/(TP+FN),TN/(FP+TN),FP/(FP+TN),FN/(TP+FN),ERR/n,RC,PR,F)}}\'>>{}'.format(fnr,k,FDextr,args.lx,etime,fnr)
#
