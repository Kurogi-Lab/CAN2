...
int main(int argc,char **argv)
{
  ...
  int seed=1;//should not be 1 ?? why
  int nop=0;
...
#define MSP 0
  ...
  for(i=3;i<argc;i++){
	...
	for(p=&argv[i][2],_nN=0;;){
	  sscanf(p,"%d",&NN[_nN++]);
	  for(;;) if(*p==','|| *p==0) break; else p++;
	  if(*p==0) break;
	  p++;
	}
      }
    ...
    else if(strncmp(argv[i],"NN:",3)==0){//NN:N1-N2:N3-N4:...
      char *p;
      _nN=0;
      for(p=&argv[i][3];;){
	if(*p==0) break;
...
//  double *skwN=(double *)malloc(sizeof(double)*(_nN));
  ...
  char *fn_predall_id="tspall_id.dat";
  ...
  if(DiffMode==1){//             dy
	...
	if((fp=fopen(fnpredlong,"r"))!=NULL && *pupdate=='0'){
	  ...
	  if(rs_method==-1){
	    ...
	    fclose(fp);
            if(1==1){
              sprintf(cmd,"./ensrs %s 2:%d:%g:%d %d-%d:1 bg:/dev/null Tsk:0:%d-%d:%d-%d",fntrain,rs_ens,rs_alpha,rs_seed,N,N,ttrainfrom+1,tmtrainto,tmpredfrom,tpredto);//????NG
		...
		sprintf(cmd,"cat predict+.dat|awk '{print ($2+%d),$3,$1;}'> %s",tpredfrom,fn_predall);
              ...
              fprintf(stderr,"#Exec1 %s ",cmd);
              system(cmd);
              sprintf(cmd,"cat loss+.dat|awk '{print $0,\"N%d\";}' > %s",N,fn_lossall);system(cmd);
              system(cmd);
              sprintf(cmd,"echo \"%d %d\" >> %s",N,seed,fn_predall_id);
              system(cmd);
              printf("#add1 '%s'(N seed) to %s ",cmd,fn_predall_id);
	      n_predall++;
	    }
	  }
	    ...
	    for(t=Tbst;t<=Tbst;t++){
              if(1==1){
                sprintf(cmd,"./ensrs %s 2:%d:%g:%d %d-%d:1 bg:/dev/null seed:%d",fntrain,rs_ens,rs_alpha,rs_seed,N,N,seed);
                for(j=3;j<argc;j++) { if(rsmo[j]==1) sprintf(&cmd[strlen(cmd)]," %s",argv[j]);}    
              }
	      else{
	    ...
	    for(i=tmpredfrom;i<tpredto;i++){
	      ...
	      for(t=Tbst;t<=Tbst;t++){
		if(1==1){
		  sprintf(cmd,"./ensrs %s -1:%s",fntrain,fnpred);
		  //		  sprintf(cmd,"./ensrs %s -1:%s seed:%d",fntrain,fnpred,seed);
		  for(j=3;j<argc;j++) { if(rsmo[j]==1) sprintf(&cmd[strlen(cmd)]," %s",argv[j]);}
		}
		else{
		...
		int initpredall=0;
                ...
                fprintf(stderr,"#Exec2 %s ",cmd);
                system(cmd);
                sprintf(cmd,"cat loss+.dat|awk '{print $0,\"N%d\";}' > %s",N,fn_lossall);system(cmd);
                system(cmd);
                sprintf(cmd,"echo \"%d %d\" >> %s",N,seed,fn_predall_id);
                system(cmd);
                printf("#add2 '%s'(N seed) to %s ",cmd,fn_predall_id);
                n_predall++;
//		if((fp=fopen(fn_predall,"r"))==NULL){
//		  initpredall=1;
//		}
...
//	}
		...
		mse,mse/mse0,msed,msed/msed0,nmse,ttrainfrom,ttrainto,tpredfrom,tpredto,k,N,T,mpn,mpd,tau_c,Hosei);
	{
	  ...
	  fprintf(fp,"#tspredv3 %e %e %e %d %d %d %d %d %d %d %d %d %d %g %d #mse,mse0,msed,nmse,tt0,tt1,tp0,tp1,k,N,T,mpn,mpd,tau_c,Hosei,it%d,seed%d\n",
		  mse,mse0,msed,nmse,ttrainfrom,ttrainto,tpredfrom,tpredto,k,N,T,mpn,mpd,tau_c,Hosei,n_tspredv3++,seed);
	  fclose(fp);
	}
	...
	if((fp=fopen(fnpredlong,"r"))!=NULL && *pupdate=='0'){
	  ...
	  if(rs_method==-1){//single CAN2
	    ...
	    //	  sprintf(cmd,"./ens2ge %s /dev/null Tsk:0:%d:%d:-%d",fntrain,tmtrainto,tpredto,ttrainfrom);
            ...
            if(1==1){
              sprintf(cmd,"./ensrs %s -1:/dev/null %d-%d:1 Tsk:0:%d-%d:%d-%d",fntrain,N,N,ttrainfrom,tmtrainto,tmpredfrom,tpredto);//20191206
              //sprintf(cmd,"./ensrs %s 2:%d:%g:%d %d-%d:1 bg:/dev/null Tsk:0:%d-%d:%d-%d",fntrain,rs_ens,rs_alpha,rs_seed,N,N,ttrainfrom+1,tmtrainto,tmpredfrom,tpredto);//????NG
              for(j=3;j<argc;j++) { if(rsmo[j]==1) sprintf(&cmd[strlen(cmd)]," %s",argv[j]);}
            }
	    else{//original
	    ...
	    if(ynormalize==0){
	      ...
	      sprintf(&cmd[strlen(cmd)]," y:%s",&argv[ynormalize][2]);
            ...
            if(nop==1) sprintf(&cmd[strlen(cmd)]," > /dev/null");
	    ...
	    for(t=Tbst;t<=Tbst;t++){
	      if(1==1){
		//		seed=5;
		sprintf(cmd,"ensrs %s 2:%d:%g:%d %d-%d:1 bg:/dev/null",fntrain,rs_ens,rs_alpha,rs_seed,N,N);//
		//sprintf(cmd,"ensrs %s 2:%d:%g:%d %d-%d:1 bg:/dev/null seed:%d",fntrain,rs_ens,rs_alpha,rs_seed,N,N,seed);//
		for(j=3;j<argc;j++) { if(rsmo[j]==1) sprintf(&cmd[strlen(cmd)]," %s",argv[j]);}
		...
		if(1==1){
		  //		  seed=6;
		  sprintf(cmd,"ensrs %s 2:%d:%g:%d %d-%d:1 bg:%s",fntrain,rs_ens,rs_alpha,rs_seed,N,N,fnpred);//
		  for(j=3;j<argc;j++) { if(rsmo[j]==1) sprintf(&cmd[strlen(cmd)]," %s",argv[j]);}
...
//	  }
	  ...
	  {//????
	    ...
	    if(n_predall==0 && initpredall==1){
	      ...
	      //	      sprintf(cmd,"cat loss+.dat|awk '{print $0,\"N%d\";}' >> %s",N,fn_lossall);system(cmd);
            ...
            fprintf(stderr,"#Exec3 %s ",cmd);
            sprintf(cmd,"echo \"%d %d\" >> %s",N,seed,fn_predall_id);
            system(cmd);
            printf("#add3 '%s'(N seed) to %s ",cmd,fn_predall_id);
	    n_predall++;
	  }
	  if((DISP&0x01)==0x01){
	    char *fnplt="dispy.plt";
	  ...
	  fprintf(stderr,"#MSE=%e(NMSE%e)(n%d) %d-%d:%d-%dk%dN%da%gb%dT%dmp%d:%d:%gH%d-y\n",mse,mse/mse0,nmse,ttrainfrom,ttrainto,tpredfrom,tpredto,k,N,rs_alpha,rs_ens,T,mpn,mpd,tau_c,Hosei);
	  {
	    ...
	    fprintf(fp,"#tspredv3 %e %e %d %d %d %d %d %d %d %g %d %d %d %d %g %d #mse,mse0,nmse,tt0,tt1,tp0,tp1,k,N,rs_alpha,rs_ens,T,mpn,mpd,tau_c,Hosei,it%d,seed%d\n",
		    mse,mse0,nmse,ttrainfrom,ttrainto,tpredfrom,tpredto,k,N,rs_alpha,rs_ens,T,mpn,mpd,tau_c,Hosei,n_tspredv3++,seed);
	    fclose(fp);
	  }
	  fprintf(stderr,"#The prediction is saved in %s.\n",fnresult0);
	}////save result
