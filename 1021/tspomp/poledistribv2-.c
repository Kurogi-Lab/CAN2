...
//#define nvowel 5//sat
  ...
  system("cat /dev/null > tmp/Mall.dat");
  for(n=0;n<nst;n++){ 
    int i;
    char buff[128];
    char fn[133];
    ...
    sprintf(&cmd[strlen(cmd)]," DISP:%s nop:%d seed:%d",DISP,nop,seed);
    ...
    for(snets[n][0]=0,j=0;j<rs_ens;j++){
      ...
      sprintf(&snets[n][strlen(snets[n])],"result-ensrs/net/%s:y%d-%d+m%da%gb%d:%ds%dj%dN%dK%d.net ",
	      snametext[n],_t0,_t1,rs_method,rs_alpha,rs_t_boost,rs_Boost,seed,j,N,k);//
    ...
    fprintf(fp,"-1          #0:timeseries,1:function approximation in my_function.c\n");
    //    fprintf(fp,"%d %d       #channels\n",k,k2);
    //    fprintf(fp,"%s/%s.dat\n",directry,snametext[n]);//fprintf(fp,"%s/%s.dat #training file\n",directry,snametext[n]);
...
//    fprintf(fp,"!cp tmp/poles5.dat tmp/%s-poles5.dat\n",sfb[n]);
  ...
  system("cat tmp/M.dat >> tmp/Mall.dat");
  
  FILE *fpout=stderr;//    fp=stdout;
  {
    fprintf(fpout,"cmd='%s'\n",cmd0);//fprintf(fpout," > /dev/null'\n");
  }
  system("cp tmp/Mall.dat tmp/M.dat");
