...
#  w = float(inet[6]) #L769
  ...
  cmd_can2='-k {} -in {}'.format(k,args.inet)
...
##
      ...
      for j in range(n_Folds):
        fnnetj ='tmp/{}+null+b{}a{}s{}N{}k{}j{}.net'.format(fn2dbe(fntrain0)[1],n_Folds,a_bag,s_bag,N,k,j)
        if not os.path.exists(fnnetj):
          ...
          fnpredj='tmp/tp{}-{}+null+b{}a{}s{}N{}k{}j{}.dat'.format(tp0,fn2dbe(fntrain0)[1],n_Folds,a_bag,s_bag,N,k,j)
...
#    import pdb;pdb.set_trace(); #for debug 
    ...
    for t in [0]:
      for j in range(n_Folds):
        fnpredj='tmp/tp{}-{}+null+b{}a{}s{}N{}k{}j{}.dat'.format(t+tp0,fn2dbe(fntrain0)[1],n_Folds,a_bag,s_bag,N,k,j)
        ypbag[t,j]=float(xp.array(pd.read_csv(fnpredj,delim_whitespace=True,dtype=str,header=None))[0,0])
#      import pdb;pdb.set_trace(); #for debug 
      yp[t+k]=xp.mean(ypbag[t,:])
    test=xp.zeros((1,k1),dtype=xpfloat)
    for t in range(1,n_pred):
      fntest='tmp/test{:03d}.dat'.format(t+tp0)
      test[0,:k]=yp[t:t+k][::-1] #reverse y(t)= M(0)*y(t-1)+...+M(k-1)*y(t-k)...+ Mk*BIAS
      test[0,k]=0 #yp[t+k+tpD-1] #dummy?
#      import pdb;pdb.set_trace(); #for debug 
      df=pd.DataFrame(test.reshape((-1,k1)))
      df.to_csv(fntest,index=False,sep=' ',header=None, float_format='%.7e')
      cmd=[]
      fnpred=[]
      for j in range(n_Folds):
#        fntrainj='tmp/train{:02d}.dat'.format(j)
        ...
        fnpredj='tmp/tp{}-{}+null+b{}a{}s{}N{}k{}j{}.dat'.format(t+tp0,fn2dbe(fntrain0)[1],n_Folds,a_bag,s_bag,N,k,j)
        fnpred.append(fnpredj)
        cmdj='python can2.py -fn {},{},{} -fnl {} -nop {}'.format(fntrainj,fntest,fnpredj,fnnetj,nop)
        cmd.append(cmdj)
#      myshell(cmdj)
#        import pdb;pdb.set_trace(); #for debug 
      with concurrent.futures.ProcessPoolExecutor(m_cpu) as excuter:
        result_list = list(excuter.map(myshell, cmd))
      for j in range(n_Folds):
        ypbag[t,j]=float(xp.array(pd.read_csv(fnpred[j],delim_whitespace=True,dtype=str,header=None))[0,0])
      yp[t+k]=xp.mean(ypbag[t,:])
...
#      import pdb;pdb.set_trace(); #for debug 
      ...
      mes='[{},{}]({:.1f}s) #[T,Tpinv] k{} N{} b{}a{}s{}m{} nop{} t{}H{}'.format(T,Tpinv,elapsed_time1,k,N[0],b_bag,a_bag,s_bag,m_cpu,nop,args.t,hpred)
      print('{}'.format(mes))
...
#    import pdb;pdb.set_trace(); #for debug 
  ...
  #        Itest_Fold.append(Itest_Foldj)
        ...
        fnpredj='tmp/{}-{}+{}+b{}a{}s{}N{}k{}j{}.dat'.format(fn2dbe(fnpred0)[1],fn2dbe(fntrain0)[1],fn2dbe(fntest0)[1],n_Folds,a_bag,s_bag,N,k,j)
  ...
  #      import pdb;pdb.set_trace(); #for debug 
    ...
    elapsed_time1 = time.time() - starttime
    mes='[{},{}]({:.1f}s) {:.3e} #[T,Tpinv] MSE n{},{} k{} N{} b{} a{} s{} m{} seed{} nop{}'.format(T,Tpinv,elapsed_time1,mse,n_train0,n_test0,k,N,b_bag,a_bag,s_bag,m_cpu,seed,nop)
    print('{}'.format(mes))
    quit()
