...
#      import pdb;pdb.set_trace(); #for debug 
      ...
      test0=xp.zeros((1,k1),dtype=object) #   test0=xp.zeros((1,k1),dtype=str)
...
#    y0=Y0
    y0=my_function.moverange(Y0,net[0]['ymin0'],net[0]['ymax0'],net[0]['ymin'],net[0]['ymax'])
#    import pdb;pdb.set_trace(); #for debug 
#move
    kD=k+tpD-1
    ...
    for t in range(0,n_pred):
      ...
      for j in range(n_Folds):
        ypbag[t,j],y2,Ypbag[t,j]=my_plinn.calc_output(net[j],x[0,:],x[0,:k]) 
#        import pdb;pdb.set_trace(); #for debug 
      Yp[t+kD]=xp.mean(Ypbag[t,:])
      yp[t+kD]=xp.mean(ypbag[t,:])
    mo=xp.concatenate((Yp[kD:].astype(str).reshape((-1,1)),xp.array([t for t in range(tp0+tpD,tp1+tpD)],dtype=str).reshape((-1,1))),axis=1)
