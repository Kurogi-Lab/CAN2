...
#  y=xp_dot(net['am']['M'][s[i],0],x) #slower?
  y=0
...
#  for t in xrange(n_x):
##    y[t]=xp_dot(net['am']['M'][int(zzs[t]),0],x[t]) 
...
#        if in_window(net['w'][sxi],net['w'][i],x[t],width,n_channels) > 0:
          y_hat_i=xp_dot(net['am']['M'][i,0],x[t])
          y_hat_xi=xp_dot(net['am']['M'][int(sxi),0],x[t])
          delta_w_ic = 0.0
          for k in xrange(n_channels):
...
#  import pdb;pdb.set_trace() #for debug
  if switchCupy.is_cupy():
    net['print']('#check S={:.3g} ent{:.3g} NDS{}{:.3g}>={:.3g}:{} entropy{:.3g}<{:.3g}:{}-> Reinit={}'.format(S.get(),net['nentropy'].get(),NDSmes,NDS.get(),NDS_thresh,reinit_flag1,net['nentropy'].get(),net['nentropy_thresh'],reinit_flag2,reinit_flag))
  else:
...
###(2)
    for i in xrange(net['n_cells']):
      if len(V['ij2t'][i])>0: 
        net['am']['M'][i]=xp_dot(xp.linalg.pinv(x[V['ij2t'][i],:]),y[V['ij2t'][i],:]).T #擬似逆行列 same as GSL in 
  else:
####(1)
    for i in xrange(net['n_cells']):
...
####      import pdb;pdb.set_trace() #for debug 
####      if i<7:
#####        print('t={} x={} y={} M={} e={}'.format(t,net['cell'][i]['am']['x'], net['cell'][i]['am']['y'], net['cell'][i]['am']['M'], y[t]-xp_dot(net['cell'][i]['am']['M'],x[t])))
####        print('i{} j{} t{} x={} y={} M={} e={}'.format(i,j,t,net['cell'][i]['am']['x'], net['cell'][i]['am']['y'], net['cell'][i]['am']['M'], y[t]-xp_dot(net['cell'][i]['am']['M'],x[t])))
####  import pdb;pdb.set_trace(); #for debug 
...
###(2)   if net['pinv']>0:
  ...
  for i in xrange(net['n_cells']):
    if len(V['ij2t'][i])>0: 
      Mi=xp_dot(xp.linalg.pinv(x[V['ij2t'][i],:]),y[V['ij2t'][i],:]).T #擬似逆行列 same as GSL in 
...
#        import pdb;pdb.set_trace() #for debug
        if switchCupy.is_cupy():
          net['print']('{} #reinit w[{}]=[{:.2g},]a{:.2g} near-to w[{}]=[{:.2g},]a{:.2g};a_{:.2g}'.format(net['GlobalTime'],rho_N_i,net['w'][rho_N_i,0].get(),net['alpha'][rho_N_i].get(),rho_i,net['w'][rho_i,0].get(),net['alpha'][rho_i].get(),net['alpha_bar'].get()))
        else:
...
#              ltemp -= net['cell'][i]['am']['M'][0][k] * x[L_sort_index[j]][k]
            ltemp = y[L_sort_index[j]]-xp_dot(net['am']['M'][i,0],x[L_sort_index[j]])
...
def net_save(net,fname):
  import six
  with open(fname, 'wb') as output:
    six.moves.cPickle.dump(net, output, -1)
...
#  fname=args.fnl
  if not os.path.exists(fname):
    ...
    quit();
#    make_and_save_data()
  with open(fname, 'rb') as net_pickle:
    net = six.moves.cPickle.load(net_pickle)
  return(net)
