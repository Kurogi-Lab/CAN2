...
#              mypltshow("tmp/fig.eps");
              ...
              if DISP[1] == '1':
                print('### Now for DISP debug mode: s,d,l={},{},{}'.format(s,d,l));

                import pdb;pdb.set_trace(); #for debug 20191105
#                Pa=P.reshape(-1,4);
#                plt.plot(Pa[:,0],Pa[:,1],'p',color='black');plt.show()
...
#            print('### Now in debug mode');
          ...
          if args.dbgi>0 and it==args.dbgi:
            print('### Now in debug mode: s,d,l={},{},{}'.format(s,d,l));
            import pdb;pdb.set_trace(); #for debug 20191105
            if 1==1:
              df=pd.DataFrame(P.reshape(-1,4));
              df.to_csv('{}/poles-{}-{}-{}.txt'.format(dr,s,d,l),index=False,sep=' ',header=None)
