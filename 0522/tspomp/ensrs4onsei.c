...
#define RANDOM 0
#define MYRAND 1
#define DRAND48 2 //compatible with 32bit and 64bit?
#define ZMTRAND 3
///
#define RAND ZMTRAND
//#define RAND RONDOM
...
//	  }
	...
	{ 
	  ...
	  fprintf(stderr,"#Mean nTestData=%g in nGivenData=%d.\n",meannTestData,nGivenData);
	}
	//	free(nr);
	//	free(nc);
	//	free(nTestData);
...
//      }
    ...
    else if(strncmp(argv[i],"sd:",3)==0) sscanf(&argv[i][3],"%d",&seed);
...
//    }
  ...
  if(nFolds>=nFoldsmax){            //nFoldsのエラー処理
    if(method==MULTIFOLD && nFolds>nGivenData ){            //nFoldsのエラー処理
      fprintf(stderr,"nFolds must be 0<nFolds<%d<nGivenData=%d. But no ristriction",nFoldsmax,nGivenData);
      nFolds=nGivenData-1;//050819??
    }
    else nFolds=nFoldsmax;
  }
    
  printf("number of given data =%d, dimension of the input vector=%d\n",nGivenData,K);
...
//	}
  ...
  {//
    ...
    if(BAGGING){//ensemble of all resampled-training folds (datasets)
      if(ibmode) sprintf(cmd2,"./meanpred %s ry:%d:%d:%.7e:%.7e:%.7e LDm:%d",ibmode,r1,r2,r3,ymin,ymax,LDmode);//r3桁で四捨五入
	...
	if(ibmode) sprintf(cmd1,"./meanpred %s ry:%d:%d:%.7e:%.7e:%.7e LDm:%d",ibmode,r1,r2,r3,ymin,ymax,LDmode);//r3桁で四捨五入
	...
	for(ii=0;ii<nens;ii++){
	  ...
	  if(((fp=fopen(fn_pred,"r"))!=NULL)&&((fp1=fopen(fn_is,"r"))!=NULL)&&((fp0=fopen(fn_pred0,"r"))!=NULL)){
	    ...
	    fprintf(stderr,"'%s' getting created.\n",fn_pred);
	    char fn_net[fnsize];
	    int fn_net_exist=0;
	    sprintf(fn_net,"result-ensrs/net/%s+m%da%gb%d:%ds%dj%dN%dK%d.net",fnbody_givendata,method,alpha,t_boost,Boost,seed,j,_nCells,K);//
	    ...
	    if(Task==TIMESERIES){
	      ...
	      //	      fprintf(fp,"0        #0:時系列,1:関数,3:ijcnn04,4:距離データ           \n");
	      //	      fprintf(fp,"%d 0     #次元 k1, k2				       \n",K);
	      fprintf(fp,"%s						       	        \n",fntrain[j]);
	      //	      if(BAGGING==NoBAG)  fprintf(fp,"%d %d %d				        \n",t1,t2,t0);
		...
		//fprintf(fp,"%e %e %e %e %e:%e:%e:%e #y正規化[ymin0:ymax0]→[ymin1:ymax1]\n",ymin,ymax,ymin1,ymax1,y0m,y1m,y0M,y1M); 
		fprintf(fp,"%d %d %e #r1 r2 					        \n",r1,r2,r3);
	      ...
	      fprintf(fp,"%d 0       #dimensionality k1 k2					\n",K);
	      //	      fprintf(fp,"1          #0:時系列,1:関数,3:ijcnn04,4:距離データ,No.1\n");
	      //	      fprintf(fp,"%d 0       #次元 k1 k2					\n",K);
	      fprintf(fp,"%s						       	        \n",fntrain[j]);
		...
		{
		  //fprintf(fp,"%e %e %e %e    出力の正規化 ymin0 ymax0 →ymin1 ymax1	        \n",ymin,ymax,ymin1,ymax1); 
		  //for(i=0;i<K;i++) fprintf(fp,"%e %e %e %e #入力%d正規化xmin0 xmax0→xmin xmax\n",xmin[i],xmax[i],xmin1[i],xmax1[i],i);
		  fprintf(fp,"%e %e %e %e %e:%e:%e:%e   #y-normalization[ymin0:ymax0]->[ymin1:ymax1]\n",ymin,ymax,ymin1,ymax1,y0m,y1m,y0M,y1M); 
		  //fprintf(fp,"%e %e %e %e %e:%e:%e:%e   #y正規化[ymin0:ymax0]→[ymin1:ymax1]\n",ymin,ymax,ymin1,ymax1,y0m,y1m,y0M,y1M); 
		  ...
		  //for(i=0;i<K;i++) fprintf(fp,"%e %e %e %e %e:%e:%e:%e #x%d正規化xmin0:xmax0→xmin:xmax\n",xmin[i],xmax[i],xmin1[i],xmax1[i],x0m[i],x1M[i],x0M[i],x1M[i],i);
		  fprintf(fp,"%d %d %e #r1 r2 					        \n",r1,r2,r3);
		}
	    }//if(Task==REGRESSION
	    ...
	    fprintf(fp,"qu         \n");
	    fclose(fp);
	    //	    fprintf(stderr,"--- param.dat ---\n");system("cat param.dat 1>&2"); fprintf(stderr,"--- param.dat ---\n");
	    fprintf(stdout,"--- param.dat ---\n");system("cat param.dat");fprintf(stdout,"--- param.dat ---\n");
	    if(fabs(BIAS-1.)>1e-10) {
	      char _cmd[40]; sprintf(_cmd,"./can2 BIAS:%g LINESIZE:%d < param.dat",BIAS,LINESIZE);
	      system(_cmd);//>>predict0.dat ??
	    }
	    else{
	      char _cmd[40]; sprintf(_cmd,"./can2 LINESIZE:%d < param.dat",LINESIZE);system(_cmd);//
	      //system("./can2<param.dat");//>>predict0.dat ??
	    }
#ifdef CHKTIME
		...
		fprintf(fp,"%d %d %e #r1 r2 					        \n",r1,r2,r3);
	      ...
	      printf("--- param.dat ---\n");
	      if(fabs(BIAS-1.)>1e-10) {
		char _cmd[40]; sprintf(_cmd,"./can2 BIAS:%g LINESIZE:%d < param.dat",BIAS,LINESIZE);system(_cmd);//>>predict0.dat ??
	      }
	      else{
		char _cmd[40]; sprintf(_cmd,"./can2 LINESIZE:%d < param.dat",LINESIZE);system(_cmd);//
...
//    }//    if(Boost==NoBoost){//no boosting
	...
	for(n=0;n<nGivenData;n++){
	  ...
	  for(j=0;j<nFolds;j++){
	    if(nc[j*nGivenData+n]!=1){//not in Dtrain out-of-bag oob?
	      ntest[n]++;
	      _ntest++;
	...
	for(nn=0;nn<nFolds;nn++){
	  ...
	  fprintf(stderr,"#%3d %13.7e %13.7e %3d %3d %13.7e N%d#n Ltest Lib bestIT Nr Lhat? N\n",
		  nn,mse[nn],msetrain[nn],is[nn],n_cells2[nn],Lhat[nn],NN[0]);//modified 051016
	}
	msetrainmean=msetrainsum/nFolds;
	fprintf(stderr,"#%10.5e %8.3e %8.3e %10.5e %d %d N%d %3d %10.5e #<Ltest Lstd Lstd2 Lib?(see ./loss+.dat) n nFolds N Nr Lhat>\n",
