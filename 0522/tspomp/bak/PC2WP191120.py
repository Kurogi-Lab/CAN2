...
#
          if DISP[0] == '1':
            ...
            qcDisp = np.rot90(qc).reshape(-1,1)
            qcbDisp = np.rot90(qcb).reshape(-1,1)
            q1Disp = np.rot90(q1).reshape(-1,1)
#              Xt=[0   ,WX/wx, 2*WX/wx,WX,WX+WX/wx,WX+2*WX/wx,WXpX0-1]
            Xtq=[j for j in range(na)]
            Xtlq=[j for j in range(na)]
            Ytq=[j for j in range(nr)]
            Ytlq=[j for j in range(nr)]
...
####
            ...
            mx=qcDisp.max()
            xaxis=[x for x in range(len(qcDisp))]
            plt.bar(xaxis,qcDisp/mx)#            plt.imshow(qcDisp/mx,cmap=cmap);
            plt.title('qc: P.c max{}'.format(mx));
            plt.colorbar(fraction=0.05,pad=0.04);
            plt.clim(-1,1)
#
            fig.add_subplot(gs[3,0])
            mx=q1Disp.max()
            plt.bar(xaxis,q1Disp/mx) #plt.imshow(q1Disp/mx,cmap=cmap);
            plt.title('q1: P.1 max{}\n'.format(mx));
            plt.colorbar(fraction=0.05,pad=0.04);
            plt.clim(-1,1)
#
            fig.add_subplot(gs[2:3,1])
            xaxis=[x for x in range(len(qcbDisp))]
            mx=abs(qcbDisp).max()
            plt.bar(xaxis,qcbDisp/mx) #plt.imshow(qcbDisp/mx,cmap=cmap);#
            plt.title('qcb:');
            plt.colorbar(fraction=0.05,pad=0.04);
            plt.clim(-1,1)
#
