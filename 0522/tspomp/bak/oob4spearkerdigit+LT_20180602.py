...
#oob4speakerdigit+ sp:fhs:fms:mkk:mko:mmt:mnh:mym tx:zero:ichi:ni:san:yon:go:roku:nana:hachi:kyu ntxi:10 k:36 mbas:2:300:1.6:1 dir:$dira s:-1 N:40
  ...
  parser.add_argument('-RX', default='', type=str,   help='mag r1:r2:...:rn for test')
...
#      myshell('make data-clean')
    ...
    for i_s,s in enumerate(S):
      for i_t,t in enumerate(T):
        if nRX>0: #test R is different from train R
          l_perm=np.random.permutation(nL)
        else:
          l_perm=range(nL)
        for i_l,l in enumerate(L[l_perm]):
          fn='{}-{}{}'.format(s,t,l)
          for r in R:
...
#    print "m,Fy=",m,Fy
    ...
    fTN=TN/float(FPTN);
    fER=ER/float(n);
    ret='{:.3f} {:.3f} {:.3f} {:.3f} {:.3f} = {} {} {} {} {} #TP,TN,FP,FN,ERR n{} m={} R{}'.format(fTP,fTN,fFP,fFN,fER,TP,TN,FP,FN,ER,n,m,args.R)
