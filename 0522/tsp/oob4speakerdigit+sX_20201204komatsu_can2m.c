...
int main(int argc,char **argv)
{
  ...
  for(i=1;i<argc;i++){
    ...
    else if(strncmp(argv[i],"s:",2)==0){//machines to learn s:-1 for all
      nsd=1;isd=i;
      sd="s";
      m_sd=m_sd1;
      mX=mX1;
...
//  }
  ...
  //  char *fn;
  char com[512];
  char *fn_predoob[2]={"predict+oob.dat","predict+oob_.dat"};
  char *fn_pred[2]={"predict+.dat","predict+_.dat"};
...
#define ZERO (1e-20)
  for(m=m1;m<m2;m++){
    ...
    fprintf(stderr,"1Executing '%s' for m=%d\n",com,m); system(com);
    sprintf(com,"cp %s %s",fn_predoob[0],fn_predoob[1]); system(com); //backup predict+oob
    //#    sprintf(com,"cp predict+oob.dat predict+0.dat"); system(com); //backup predict+oob
	...
	fprintf(stderr,"2Executing '%s'\n",com); system(com);
      ...
      sprintf(com,"cp %s %s",fn_pred[0],fn_pred[1]); system(com); //backup predict+oob
      //      sprintf(&com[strlen(com)],"y:-1:1:-1:1 x:0:0:0:1 DISP:0 bg:/dev/null > /dev/null");//
    }
  
    FILE *fp_predoob=fopen(fn_predoob[1],"r");//prediction of traindata by oob
    FILE *fp_pred=fopen(fn_pred[1],"r");//prediction of test(sX) data by bag
    ...
    for(s=0;s<nS;s++){
      if(s==sX){
        fp=fp_pred;
        //        printf("#%s is used. for s=%d. ",fn_pred[1],s);
      }
      else{
        //        printf("#%s is used. for s=%d. ",fn_predoob[1],s);
        fp=fp_predoob;
      }
      for(tx=0;tx<nD;tx++){//test of tx-th multi-step identification/recognition
        for(txi=0;txi<nL;txi++){//test for txi-th indexed tx-th text(digit)
...
//      }
    ...
    //    fprintf(fpw,"#%d sm\n",sm);
    {
      double n=TP+TN+FP+FN,TPFN=TP+FN,FPTN=FP+TN,ER=FP+FN;
      double TPR=TP/(TP+FN+ZERO);
      double FNR=FN/(TP+FN+ZERO);
      double FPR=FP/(TN+FP+ZERO);
      double TNR=TN/(TN+FP+ZERO);
      double ERR=(FP+FN)/(TP+TN+FP+FN+ZERO);
      double RC=TP/(TP+FP+ZERO),PR=TP/(TP+FN+ZERO); //recall and precesion
      double F=2.*RC*PR/(RC+PR+ZERO);
      if(nsd==1){//speaker identification
        sm=m;
        sprintf(com,"%.3f %.3f %.3f %.3f %.3g = %.0f %.0f %.0f %.0f %.0f %.0f %d #TPR,TNR,FPR,FNR,ERR=TP,TN,FP,FN,ER,n,sm%d ",TPR,TNR,FPR,FNR,ERR,TP,TN,FP,FN,ER,n,m,m);//m,m is necessary:first m is for speakerdigit_v8, second m is for display to the user
        //        sprintf(com,"%.0f %.0f %.0f %.0f %.0f %.0f #TP,TN,FP,FN,ER=...#n,sm%d ",TP,TN,FP,FN,ER,n,m);
      }
      else if(nsd==2){//digit identification
        txm=m;
        sprintf(com,"%.3f %.3f %.3f %.3f %.3g = %.0f %.0f %.0f %.0f %.0f %.0f %d #TPR,TNR,FPR,FNR,ERR=TP,TN,FP,FN,ER,n,dm%d ",TPR,TNR,FPR,FNR,ERR,TP,TN,FP,FN,ER,n,m,m);//m,m is necessary:first m is for speakerdigit_v8, second m is for display to the user
      }
      else if(nsd==3){
        sm=m/nD;txm=m%nD;
        sprintf(com,"%.3f %.3f %.3f %.3f %.3g = %.0f %.0f %.0f %.0f %.0f %.0f %d %d #TPR,TNR,FPR,FNR,ERR=TP,TN,FP,FN,ER,n,sm%d,dm%d",TPR,TNR,FPR,FNR,ERR,TP,TN,FP,FN,ER,n,sm,txm,sm,txm);
      }
...
//      if(nsd==1){//speaker identification
//        sm=m;
//        sprintf(com,"%.3f %.3f %.3f %.3f %.3e %.3f %.3f %.3f %d #TP,TN,FP,FN,ERR,RC,PR,F,n,sm%d",TP,TN,FP,FN,ER,RC,PR,F,n,sm);
//      }
//      else if(nsd==2){//digit identification
//        txm=m;
...
//      }
      ...
      if(sX>=0){
        ...
        double TPRX=TPX/(TPFNX+ZERO);
        double FNRX=FNX/(TPFNX+ZERO);
        double FPRX=FPX/(FPTNX+ZERO);
        double TNRX=TNX/(FPTNX+ZERO);
        double ERRX=(FPX+FNX)/(nX+ZERO);
        sprintf(&com[strlen(com)], " sX%d(TP%.3f TN%.3f FP%.3g FN%.3g ER%.3g n%.0f)",sX,TPX,TNX,FPX,FNX,ERX,nX);
      }
      fprintf(stdout,"%s\n",com);
      fprintf(fpw,"%s\n",com);
      fprintf(fps,"%s\n",com);fflush(fps);
